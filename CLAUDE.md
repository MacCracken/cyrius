...
build/               Generated binaries (gitignored except `cycc` +
                     ...
                     above); CI / install bootstrap from it.
                     `cycc-native-aarch64` is the ONE cross-bin that's
                     tracked, because it's the only one that can't be
                     ...
                     CI / install and stay ignored. **cc3 (the v5.0.0-era
                     prior-prior seed) was DROPPED at v6.1.0** — it was never
                     in the bootstrap chain (seed = `bootstrap/asm` → `cybs`
                     → `cycc`) and nothing in build/CI loaded it; the earlier
                     "drops at v7.0.0" plan was retired early at the clean
                     cut. No prior-seed slot is needed going forward: `cycc`
                     is forever, so there are no name changes for fresh
                     checkouts to bridge.
