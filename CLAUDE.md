...
### Compliance / external
9. **Security re-scan** — quick grep for new `sys_system`, `READFILE`, unchecked writes. Full audit every 2-3 minors. **Last full audit: `docs/audit/2026-09-03-security-audit.md` (CVE-38…CVE-42) at cycc 6.5.45**; before it `docs/audit/2026-07-27-security-audit.md` (CVE-32…CVE-36) at cycc 6.4.82, and `docs/audit/2026-06-10-deep-dive-review.md` (…CVE-31) at cycc 6.1.31. *(This line read "last: v5.0.1" until v6.4.82 — three minors stale, which is how the re-scan cadence quietly slipped — and then pointed at the JULY audit through the whole of v6.5.x while a September one existed. Corrected at the v6.5.73 closeout.)* ⚠ **The next CVE number is 46.** The July file carries `CVE-37` and `CVE-38` marked **(withdrawn)** — withdrawn findings still consume their identifier — the September file runs 38…42, **`CVE-43` was consumed at 6.6.5** (the dep-cache tamper check that trusted the cache's own index and failed OPEN around it) and at 6.6.6 both **`CVE-44`** (the release installer staged a tarball and its signature inputs at fixed `/tmp` names) and **`CVE-45`** (an INCLUDED file, a `#define` macro body and a `#derive` tail could each forge a `#@file` marker and defeat `private` visibility); all are appended to the September file. So anything below 45 collides. *(This counter is only ever correct if the bite that spends an id updates it in the same commit — 6.6.5's did not, and three separate reviewers had to report it.)*
