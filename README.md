...
| Metric | Value |
|--------|-------|
| Compiler (`cycc`) | **1,103,512 B** (~1.05 MB) x86_64 at v6.4.72 |
...
| External dependencies | **0** at the compiler level (0 git deps at stdlib level: mabda folded, now 4.0.7) |
| Tests | **251** .tcyr + **6** .fcyr fuzz + **18** .bcyr bench + 1 .scyr soak + 1 .smcyr smoke |
| Gates (`scripts/check.sh`) | **147** structural + runtime gates + QEMU kernel boot gate (incl. PIE exec gate at v6.1.6, QEMU kernel boot gate at v6.2.28, deps-resolver gates `_deps_requires`/`_deps_sidecar`/`_deps_groups`/`_deps_modular` @ v6.2.46–.50, CVE-32 modular-traversal gate @ v6.2.51, the 8300-var `_var_grow_gate` @ v6.3.0, the lever-2 `_deps_features_gate` @ v6.3.1) |
...
| Stdlib modules | **99** (distfiles folded byte-identical; bayan 1.2.1 → `lib/bayan.cyr`, ganita 1.0.4 → `lib/ganita.cyr`, `lib/sys.cyr` system-introspection @ v6.1.28; see [docs/stdlib-modules.md](docs/stdlib-modules.md)) |
...
| 3. Full compiler | `cycc` | ~1.05 MB |
| 4. Linker | `cyrld` | 908 KB |
...
**~74 total** lexer-reserved tokens. C23 has 59 keywords for comparison. The math intrinsics (~32) are exposed as keywords because they emit specific instruction sequences and dispatch per-backend (x86 SSE/AVX2, aarch64 NEON, cx bytecode per-lane loops); in C those would be `__builtin_*` or library calls.
...
at 4.0.7, dropping its transitive `agnosys` and leaving zero `[deps.*]`
...
src/
  ...
  backend/x86/          x86_64 emission (emit/jump/fixup/decode) + float.cyr = SSE/AVX scalar-f64 + ALL SIMD emitters; dispatches ELF/PE/Mach-O containers
...
bootstrap/asm (29,024 B committed binary -- root of trust)
  -> cybs (12,344 B compiler)
    -> cycc (modular compiler + IR, 1,103,512 B at v6.4.72)
