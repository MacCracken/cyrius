# Cyrius Compiler Benchmarks — v5.6.x Optimization Arc (Historical)

> **Frozen historical artifact.** This page captures the v5.6.x
> optimization arc (v5.6.5–v5.6.27) in detail because that arc was
> the project's single largest perf push and its per-patch deltas are
> still load-bearing context for understanding cyrius's codegen
> shape. It is NOT the current-state benchmark surface.
>
> **For CURRENT bench data**: [`/BENCHMARKS.md`](../../BENCHMARKS.md)
> at the repo root — auto-generated by `scripts/bench-history.sh`
> (3-tier suite: stdlib primitives / data structures / compiler).
> Run history in `bench-history.csv`. Cadence: every release closeout
> + on-demand regression checks.
>
> **For v5.7.x → v5.11.x perf events**: CHANGELOG narrative entries
> (e.g. [5.10.40] lex dedup −90 %, [5.10.41] fixup hash-table −64 %).
> No standalone per-arc summary doc — CHANGELOG is canonical per
> CLAUDE.md.
...
Workload: self-host compile (`cycc < src/main.cyr > /dev/null`). This
...
## v5.6.x series — output-byte count on cycc itself

When a peephole shrinks generated code, the cycc binary (which is
generated by cyrius compiling its own source) shrinks too. This is
a more stable signal than compile wall-clock.

| Release | cycc size (B) | Δ vs v5.6.4 | Note |
|---------|--------------|-------------|------|
...
0 B with flag reuse) cascades across thousands of sites in cycc's
own source.
...
bytes), the standard "build cycc; rebuild with cycc; compare" check
doesn't hold across the compiler-upgrade step. The correct pattern
is:

```sh
cat src/main.cyr | build/cycc > /tmp/cc5_a; chmod +x /tmp/cc5_a
...
cycc records `clock_gettime(CLOCK_MONOTONIC)` at entry (after env
...
```sh
CYRIUS_PROF=1 build/cycc < src/main.cyr > /tmp/cc5_out 2> /tmp/cycc.prof
cat /tmp/cycc.prof
# → prof: compile 400 ms
```
...
`cycc_aarch64` uses syscall 113 (aarch64 Linux generic table number for
...
shell-level timing (`time cycc_win.exe < ...`) is the recommendation
