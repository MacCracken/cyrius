...
**Status:** 🟡 **OPEN** — newly filed. Verified against live code at 6.5.35 on 2026-08-23: `ESYSXLAT`'s ELF-aarch64 arm (`src/backend/aarch64/emit.cyr`) has **no `16→29` row**, and the `syscall not routed` warning (`src/frontend/parse_expr.cyr:954`) is still gated to the Mach-O branch. Demonstrated end-to-end under `qemu-aarch64`: the shadowed build returns `-EFAULT` where the clean build returns `0` on a real tty.
