> ✅ **RESOLVED — v6.1.31 (fold sigil 3.7.9).** Premise corrected: `tls_native`'s
> CertVerify sign + verify already handled Ed25519. The real bug was one layer down —
> **sigil's `x509_parse` was ECDSA/RSA-only**, so the server's `load_creds` rejected the
> Ed25519 cert (`x509_parse` → CERT_INVALID) before the handshake. sigil 3.7.9 added
> Ed25519 X.509 leaf-cert support (RFC 8410: id-Ed25519 sig-algid, raw 32-byte SPKI key,
> PureEd25519 `_x509_verify_link`); folded into `lib/sigil.cyr`. Verified: native loopback
> handshake + **OpenSSL `s_client` interop** (`Peer signature type: ed25519`, server
> accepted). `tests/tcyr/tls_native_ed25519.tcyr`. sit can now broaden its TLS CI to
> Ed25519 server certs. See CHANGELOG [6.1.31].
