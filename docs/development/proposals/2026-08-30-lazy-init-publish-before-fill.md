...
**Status:** ✅ **COMPLETE at v6.5.39 — both halves shipped.** Option (a) (fill a local, publish last) shipped at v6.5.37 in `_chrono_init_mdays`; the memory-ordering half shipped here as an aarch64-gated release fence at three publish sites. Gate: `tests/gates/concurrency/lazy_init_release_fence.sh` (mutation-proven three ways).

⚠ **Three corrections this proposal got wrong, all load-bearing for whoever reads it next.**
1. **It named `lib/alloc.cyr:89-98` as the model. That is the OLD shape** — it fences on BOTH arches, and x86-TSO already orders store-store, so the x86 fence is pure tax (measured ~22 ns, 83 % of the lock cost). The project already made this call and moved on: `lib/freelist.cyr:128-134` fences under `#ifdef CYRIUS_ARCH_AARCH64` only. Copying alloc verbatim would have re-added the tax that was deliberately removed. The shipped fix copies freelist.
2. **`lib/tls.cyr:211` is the wrong site.** `_tls_inited` is an *attempt latch*, set before the resolution work; readiness is `_tls_ok`, published last. The proposal also **missed a second site entirely**, `_tls_introspect_resolved`. Both are now fenced; `:211` correctly is not.
3. **`lib/regex.cyr:225` is deliberately NOT fenced.** It publishes a *flag*, not a pointer, so the reader has no address dependency and a writer-side fence would not suffice — and the matcher it guards is non-reentrant process-global state regardless, so fencing its init would fix nothing observable while manufacturing a false thread-safety signal.

⚠ **"The include cost is part of the decision" was false** — measured at **zero bytes** on x86-linux, PE and the bare-metal kernel fixture, because `alloc.cyr` already pulls `atomic.cyr` and includes are once-only.

⚠ **No runtime test is possible, and that was proven rather than assumed:** under `qemu-aarch64` the known-broken pre-.37 mutant scores 0 bad runs in 150, i.e. qemu does not exhibit the interleaving at all. The property is therefore pinned at the codegen level.
