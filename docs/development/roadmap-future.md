...
| **Async single-waiter-per-fd** | — | ✅ **SHIPPED — no longer a watching item.** ⛔ This row read *"▲ BOUND INTO the v6.5.x Slot 8 coroutine arc … **Verified still live at v6.5.19**"* until 2026-09-02, and that verification had been overtaken by shipped code. Re-derived from live source: `_async_wait_events` (`lib/async.cyr:334`) computes a UNION mask via `_async_fd_mask` (`:322`) and falls back ADD→MOD; `_async_step` walks the ENTIRE task list (`:201-211`) waking every task parked on that fd whose mask matches, `EPOLL_CTL_MOD`s the remainder rather than unconditionally DEL-ing (`:217-230`), and wakes ALL waiters on EPOLLERR/EPOLLHUP (`fired & 24`, `:200`) since a peer that closes mid-relay delivers HUP only. The per-task `wait_ev` field is at +56. ⚠ The double-tracking this row described is therefore resolved. **Its remaining half — Slot 8's Half B, mid-body suspend / CPS — SHIPPED at v6.5.69** (see the struck row above); the pointer to `roadmap.md` that stood here dangled once the v6.5.x slot list retired at the 2026-09-08 rewrite. Nothing of this item is outstanding. |
| **Unicode NFKC/NFKD compatibility forms** | Low | ▽ **WATCHING — no consumer has asked.** `lib/unicode/normalize.cyr` implements the CANONICAL forms (NFC/NFD) only; the `NormalForm` enum and the output buffer are already sized for all four, so the gap is the ~445 KB compatibility decomposition table, not the algorithm. ⚠ Surfaced 2026-09-02 not by a consumer but by `cyrius audit` reaching `lib/unicode/` for the FIRST TIME (v6.5.42 gave the audit walkers recursive descent) — cyrlint flagged the in-code deferral as untracked, correctly: it had been deferred "to a later slot" in a comment with nothing pointing at it. This row is that pointer. Pull it forward when something actually needs K-forms. |
