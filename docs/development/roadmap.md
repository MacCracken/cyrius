...
| **v6.2.26** ⏳ | **agnos-fs ABI substrate** (user-pinned 2026-06-19). Length-carrying **portable syscall wrappers** (`xopen`/`xstat`/`xunlink`/`xgetdents`) beside `io.cyr`'s `file_open`; migrate the ~58 hand-rolled raw-ABI `sys_open`/stat/getdents sites onto the one bridge; extend **cyrlint** (stat/getdents arms); add an **agnos emit-inspect probe**. Bottom-of-stack; `#ifdef`-gated → x86 cycc byte-identical. Kills the silent-ABI-miscompile class for every agnos-bound caller + feeds the .27 kernel-freestanding-TLS arc. Discharges `2026-06-18-stdlib-native-agnos-abi-fs` + the partial var-syscall `ERR_*`/`SYS_*` namespace carry-in. **Premise-check first** ([[feedback_premise_check_at_slot_entry]]): fs.cyr/cyrlint pieces landed piecemeal .23/.24 — confirm what wrapper surface already exists before scoping. | bottom — agnos/stdlib |
| **v6.2.27** ⏳ | **Bare-metal target formalization** (user-pinned 2026-06-19) — codify the ad-hoc bare-metal mode (agnos has used since first boot) into a first-class `--target <arch>-bare-metal-elf` triple. 7 deliverables: target triple, ELF no-libc output (no PT_INTERP/DT_NEEDED/libc-init `_start`), interrupt-handler emit conventions (`naked_fn`), no-runtime-assumptions codegen, **kernel-freestanding TLS linkage** (the AGNOS-kernel crypto prereq — consumes the v6.2.4 transport vtable), linker-script/section control, and the cross-OS + agnos boot acceptance gate. The .26 agnos-fs substrate is its natural prereq. | bare-metal |
| **v6.2.28** ⏳ | **VR-01/VR-02 verification fold** (user-pinned 2026-06-19) — close the verification gaps the .25 Windows break exposed: copy the full-`.tcyr` loop into the **arm-native CI job** (~zero porting cost, 3× on-hardware coverage), wire **`cyrius fuzz` → a check.sh gate**, and add a **CLI cross-compile check** (`CYRIUS_TARGET_WIN`/`_MACOS`/`_AARCH64` on `cbt/cyrius.cyr` — the exact class the .25 break slipped through). From `2026-06-10-verification-coverage-gaps`. | verification/CI |
| **v6.2.29** ⏳ | **CVE-21 release / trust-chain integrity** (user-pinned 2026-06-19) — seed-disjoint trust root + fail-closed release path (`install.sh:373` still "continuing anyway" on a bad checksum). From `2026-06-10-release-trust-chain-integrity`. | security |
