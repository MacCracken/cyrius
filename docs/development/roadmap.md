...
| **v6.1.31** ✅ | **Ed25519 server certs for native TLS** (sigil 3.7.9 fold) — root cause was sigil's `x509_parse` (ECDSA/RSA-only), NOT the TLS layer; adds Ed25519 X.509 leaf-cert parse (RFC 8410, PureEd25519). Loopback + OpenSSL `s_client` interop verified. cycc unchanged. | TLS (sit-filed) |
| **v6.1.32** ✅ | **agnos argv capture → entry-landing r15 park** (Phase F) — `var r=main()` is a gvar-init so `call main` ran inside EMIT_GVAR_INITS before the v6.1.14 after-gvar capture → `argc()==0`; park r15 at the landing (mirrors v6.1.30 macho). Closes attn11's issue. cycc −176 B; ecb/cass/pi SELFHOST_OK. | F — hardening (attn11) |
| **v6.1.33** ✅ | **dep-resolver/include injection class CVE-14/15/16** (Phase F F1a) — three shell sinks → argv `execve`: `_sha256sum_file` `/bin/sh` (CVE-14, auto-runs every build), `git clone` `sys_system` (CVE-15), absolute includes (CVE-16). cycc +600 B; check.sh 87/87; ecb/cass/pi SELFHOST_OK. | F — hardening (security) |
| **v6.1.34** ✅ | **live silent-failure regressions CVE-22 + CO-03** (Phase F F1b pt1) — `_vec_die`/`_hm_die` self-recursion → SIGSEGV not exit(1) (restore `syscall(60,1)`); DSE/LASE/#regalloc x86 byte-passes ran ungated on aarch64 (gate `_AARCH64_BACKEND==0`). cycc +160 B; check.sh 87/87; ecb/cass/pi SELFHOST_OK. | F — hardening |
| **v6.1.35** ✅ | **live silent-failure regressions CVE-23 + CVE-31 + CO-02** (Phase F F1b pt2 — closes the F1b cluster) — shared `_check_output_cap` pre-write guard on all 5 unguarded ELF/PE writers (CVE-23); frontend input validation: missing-include `-1` loud / unknown-byte lex error / `file_map` 128→1024 relocated to `0x71A000` (CVE-31, surfaced + fixed sigil 3.7.10 redundant-include silent-skip); `check.sh` tcyr exit-masking → `regression_exec_capture_status` + `_input_validation_rejects_gate` (CO-02). Pre-cut adversarial review caught + fixed two latent gaps (ELF cap guards checked PT_LOAD `filesz` not the true on-disk size incl. section headers → all ELF emitters now check `shdr_off + shnum*shentsize`; reject gate now asserts nonzero exit). cycc +3,376 B; check.sh 88/88; ecb/ach/pi/cass SELFHOST_OK. | F — hardening |
| **v6.1.36** ✅ | **TLS-authn hardening (Phase F F2)** — CVE-17 (x509 pathLen/keyUsage/EKU enforcement; sigil 3.7.12 parse + cyrius `tls_native_client_verify_chain`), CVE-18 (fail-closed `connect` verify + iPAddress-SAN), CVE-30 (post-handshake NST/KeyUpdate drain), CVE-19 (ws/sandhi/NST entropy fail-closed via `sys_getrandom`; Win/AGNOS stubs). + distlib inliner fix (sigil 3.7.11) + cyrius `_vendored_dist_selfcontained_gate`. stdlib-only (cycc byte-identical); check.sh 89/89; ecb/ach/pi/cass SELFHOST_OK. | F — hardening |
| **v6.1.37** ✅ | **const chained-multiply miscompile** (cyrius-doom report) — `A * B * C` with const A,B folded to `B * C` (first operand dropped: `320*200*4`→800). NOT agnos-only (all targets); root cause = stale `_cfo` const-fold tracking after a runtime EIMUL in `parse_expr.cyr`. Fix: clear `_cfo` after the EIMUL in the 3 `_cfo==1` sub-branches. cycc +112 B; check.sh 89/89; ecb/ach/pi/cass SELFHOST_OK; x86+agnos+aarch64 verified. | bug bandwidth (cyrius-doom) |
| **v6.1.38** ✅ | **memory-safety parity (Phase F F3) — CVE-25/26/27/28 + AR-03** (5 of 6; CVE-24 deferred) — agnos `alloc()` pre-lock size guards (CVE-26), `_sb_grow` OOM propagation + `_sb_die` (CVE-25), aarch64 atomics `ldaxr`/`stlxr` acquire-release barriers + vtable-publish fence (CVE-28), PE import-registry count+name-buffer bounds (CVE-27), fixup cap 4× under its 16 MiB region unified 262144→1048576 + `jump_target_tbl` off-by-one fixed across all 4 accessors (AR-03). Adversarial-review-the-diff caught the jump fix initially touching only 1 of 4 accessors (self-host-invisible). **CVE-24 deferred** — audit premise wrong (`local_cnt` counts frame *slots* not vars; naive cap breaks 16 KB `stack var` frames) → re-scoped to [slot-indexed-overflow](issues/2026-06-12-locals-table-slot-indexed-overflow.md). cycc +736 B; check.sh 89/89; CVE-28 verified on real pi; ecb/ach/pi/cass SELFHOST_OK. | F — hardening |
| **v6.1.39** ✅ | **diagnostic `syscall` byte-length audit** — swept all 539 `syscall(WRITE,…,"…",LEN)` strings in src/+lib/; fixed 27 wrong `LEN` (16 em-dash undercounts → truncated messages; 11 off-by-one incl. over-counts → `write` over-reads adjacent `.rodata`). Data-only → cycc byte-identical, size unchanged. check.sh 89/89; ecb/ach/pi/cass SELFHOST_OK. [diagnostic-byte-length-audit](issues/2026-06-12-diagnostic-syscall-byte-length-audit.md) (notes a recurrence-prevention gate as follow-up). | hardening |
| **v6.1.40** ✅ | **CVE-24 grow-and-cap (Phase F F3 6/6 — pack COMPLETE)** — the 4 slot-indexed per-fn local tables (indexed by frame SLOT; a `stack var buf[N]` registers N/8 fillers) silently overran on frames >~2 KB. Relocated to the heap top `0x5D9D000`+ + grown to 16384 slots (128 KB frames) each; S-region +512 KB (7 driver sites); `SFLC` caps at 16384 (loud vs silent overflow). big_frame round-trips; new reject-gate case (3/3); cycc +160 B; check.sh 89/89; ecb/ach/pi/cass SELFHOST_OK. Closes the 2026-06-10 deep-dive urgent set. [archived](issues/archived/2026-06-12-locals-table-slot-indexed-overflow.md) | F — hardening |
| **v6.1.41** ✅ | **pre-v6.2.0 closeout hardening** — closeout audit caught 3 residuals: (P1) Mach-O output-cap parity (CVE-23 missed both `EMITMACHO_*` writers → large-globals macOS program overran output_buf into the .40 local tables; now `_check_output_cap` on the true size), (P2) security-reject `return 0`→`-1` (hostile `include "/etc/passwd"` exited 0; now fatal; reject-gate 4/4), (P1·latent) IR counters relocated out of fixup_tbl. + byte-length over-count `.39` missed (multi-line), dead `GFVA` removed (63→62, −256 B), heap-map doc-rot. check.sh 89/89; ecb/ach/pi/cass SELFHOST_OK. | hardening |
| **v6.1.42** ⏳ | **v6.1.x closeout pass → v6.2.0** — vidya/doc sync, downstream cyrius.cyml pin check, bootstrap-closure verify, doc-health ledger. (The daimon "codegen bug" was RE-DIAGNOSED — disasm showed `var parts[4]` = 8 bytes = the documented N-bytes *local* convention, not a `(N-1)*8` under-reserve; daimon's `store64(&local+i*8)` uses the *global* N-slots idiom on a local. Working-as-documented → moved to v6.2.x Language Refinements as a convention decision, NOT a v6.1.x codegen patch.) [addr-taken footgun, re-diagnosed](issues/2026-06-11-addr-taken-local-array-static-underreserve.md) | closeout |
| **v6.2.0** ⏳ | **Platform Expansion minor opens** — Phase 0 growable-region foundation, bare-metal, RISC-V rv64 (see [roadmap_6.md](roadmap_6.md)). Carries: local-array convention footgun (Language Refinements), permanent syscall-write byte-length gate (.39), undefined-fn-call-should-hard-error (the silent-ud2/SIGILL class). | → v6.2.x |
...
| Pack | Items | Issues |
|---|---|---|
| **F1a — dep-injection** ✅ **v6.1.33** | `deps --verify`/`--lock` shell injection (CVE-14 — auto-runs every build), git arg-injection (CVE-15), absolute-path include (CVE-16) → all argv `execve`, `--` separator, leading-`/` reject. | [archived](issues/archived/2026-06-10-deps-resolver-injection-class.md) |
| **F1b pt1** ✅ **v6.1.34** | `_vec_die`/`_hm_die` self-recursion → SIGSEGV (CVE-22, restore `syscall(60,1)`); DSE/LASE/#regalloc x86 byte-passes ungated on aarch64 (CO-03, gate `_AARCH64_BACKEND==0`) | [archived](issues/archived/2026-06-10-live-silent-failure-regressions.md) |
| **F1b pt2** ✅ **v6.1.35** | shared `_check_output_cap` pre-write guard on all 5 unguarded ELF/PE writers (CVE-23); frontend input validation — missing-include `-1` loud / unknown-byte lex error / `file_map` 128→1024 relocated to `0x71A000` (CVE-31, surfaced+fixed sigil 3.7.10 redundant-include silent-skip); `check.sh` exit-masking → `regression_exec_capture_status` + `_input_validation_rejects_gate` (CO-02) | [archived](issues/archived/2026-06-10-live-silent-failure-regressions.md) |
| **F2 — TLS authn hardening** ✅ **v6.1.36** | chain-verify gaps pathLen/EKU/keyUsage (CVE-17 — sigil 3.7.12 parse + cyrius walker enforce; revocation deferred to v7 short-cert/OCSP), connected-but-unverified + `host==0` skip → fail-closed `connect` + iPAddress-SAN (CVE-18), post-handshake NST/KeyUpdate false-EOF → drain loop (CVE-30), entropy fail-weak ws/sandhi/NST → `sys_getrandom` fail-closed + Win/AGNOS stubs (CVE-19; real Win primitive + AGNOS syscall filed). + distlib inliner fix (sigil 3.7.11) + dist self-contained gate. | [archived](issues/archived/2026-06-10-tls-chain-verification-gaps.md) · [archived](issues/archived/2026-06-10-tls-post-handshake-false-eof.md) · [archived](issues/archived/2026-06-10-entropy-failweak-paths.md) |
| **F3 — memory-safety parity** ✅ **v6.1.38 + .40** (6/6 COMPLETE) | `_sb_grow` OOM (CVE-25), `alloc_agnos` size guard (CVE-26), PE import-registry bounds (CVE-27), aarch64 atomics `ldaxr`/`stlxr` barriers + vtable fence (CVE-28), fixup cap unify 262144→1048576 + `jump_target_tbl` off-by-one all-4-accessors (AR-03) — all .38; **CVE-24 locals-table grow-and-cap** (slot-indexed tables relocated to heap-top + grown to 16384 slots + `SFLC` cap) — .40. Adversarial review caught the jump fix touching only 1 of 4; CVE-24 re-scoped after the .38 audit-premise miss. | [archived](issues/archived/2026-06-10-memory-safety-parity-gaps.md) · CVE-24 [archived](issues/archived/2026-06-12-locals-table-slot-indexed-overflow.md) · CVE-28 from [unreviewed-dimensions](issues/2026-06-10-unreviewed-dimensions.md) (LEGAL-01/CVE-29/DX open) |
