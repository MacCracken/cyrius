...
| **v6.3.10** | **Generic STRUCTS + non-i64 fn instantiation + explicit syntax (F-structs)** — second half of the generics arc (user 2026-06-30; absorbed v6.3.9's deferred non-i64 + explicit, since both reuse the instantiate-once engine). **Instantiate-once** (closure-style jump-over emit): at a non-i64 generic call mint `foo$<ty>`, dedup via `FINDFN`, emit the specialized body ONCE (replay base tokens with `T→concrete`) + normal call. **Explicit `foo<i64>(x)`** via a non-consuming `<`-disambiguation lookahead (TS-parser heuristic). **Generic structs**: capture `struct Box<T>{...}` type-params + body-token range (new `_struct_bodystart/end`), mint `Box$ty` via `REGSTRUCT`+`FINDSTRUCT`-dedup, replay fields with `T→concrete` via `ADDFIELDTYPED` (STRUCTSZ/FIELDOFF lazy → free); **nested/recursive** (`Outer<Inner<T>>`, register inner sid before outer field) + cycle guard; **generic-struct × generic-fn** (`id<Box<i64>>`). **HARD self_compile + cap-headroom + dedup-proof gates** (objdump: one body per `(fn,type-args)`). | — | ~7 |
