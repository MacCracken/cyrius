...
| .91 | **closeout + docs sweep** — CLAUDE.md Closeout Pass § (mechanical fail-fast → judgment passes → doc sync; reconcile the slot-number drift this long cycle accreted). **Includes:** the .90 review's residual stale-comment sweep — repoint refs to the deleted `programs/check.cyr` in `lib/regression.cyr` (×4), `lib/fs.cyr` (×2), `lib/audit_walk.cyr`, the probe programs (`efi_probe`/`efi_fncall_probe`/`agnos_emit_probe`/`win_emit_probe`/`win_process_probe`/`ts_test_runner`), and `tests/tcyr/gvar_static_init.tcyr` → the per-suite files. **Latent to fold:** macho-arm socket *family* (socket/connect/accept) mapped with x86 nums in the aarch64-macho ESYSXLAT but arm-macho uses aarch64-Linux nums → real-network sockets likely broken on arm-macho (cf. sandhi-connect issue). | |
