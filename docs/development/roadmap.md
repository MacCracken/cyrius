...
| **v6.1.42** ⏳ | **codegen: address-taken local-array static under-reservation (HIGH)** — `var a[N]` escaping → promoted to static but reserved `(N-1)*8` not `N*8`, corrupts the adjacent static literal (daimon: 404'd every route). Confirmed on .41; fix before the v6.2.0 minor (user 2026-06-12). [addr-taken-local-array-static-underreserve](issues/2026-06-11-addr-taken-local-array-static-underreserve.md) | bug bandwidth (daimon) |
| **v6.1.43+** ⏳ | **dep-fold cycle-close → v6.2.0** — Phase F complete. Follow-up: permanent check.sh gate for syscall-write byte-lengths (.39 audit). Bug bandwidth as filed. | → v6.2.0 |
