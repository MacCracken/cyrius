...
| **v6.3.9** | **Generic FUNCTIONS — monomorphization (F-fns)** — type-param recognition + emit-time TYPE substitution + instantiation dedup for `fn foo<T>(...)`, on the proven AR-01 token-replay substrate (`_MONOMORPH_OK`, default-off → cycc byte-identical). **Inference AND explicit `foo<i64>(x)`** call forms (user 2026-06-30): infer concrete arg types via the existing overload-dispatch lookups; explicit type-args via a non-consuming `<`-disambiguation lookahead (borrowed from the TS parser). Mint `foo$ty` into the name pool; **FINDFN's hash IS the dedup index** (one specialized fn per `(fn,type-args)`, emitted once, called normally — NOT re-inlined). Scoped `T→concrete` binding consulted at the 4 type-resolution sites (param-type/local `:T`/`slice<T>`/`sizeof`). **+ btokens ceiling 16→32** (pure constant; pc≤2 + no-control-flow gates stay). ~10 bites. **HARD self_compile delta budget + cap-headroom + dedup-proof as release gates** (perf-refactor is v6.5.x — cost can't defer). First half of the 2-release generics arc (user 2026-06-30: generic STRUCTS split to v6.3.10 — distinct higher-risk replay engine). | — | ~10 |
| **v6.3.10** | **Generic STRUCTS — monomorphization (F-structs)** — second half of the generics arc (split from v6.3.9, user 2026-06-30). A NEW struct-body token-replay engine (none exists today — only fns replay): capture `struct Box<T>{...}` type-params + body-token range (new `_struct_bodystart/end` tables), mint `Box$ty` via `REGSTRUCT` + `FINDSTRUCT`-dedup, replay the field decls with `T→concrete` ftype via `ADDFIELDTYPED` (STRUCTSZ/FIELDOFF are lazy → free). **Nested/recursive** (`Outer<Inner<T>>` — register inner sid before outer field) + cycle guard; **generic-struct × generic-fn interaction** (`id<Box<i64>>`). The hardest hazards (recursion ordering, cycle detection) isolated here so a struct bug can't entangle the v6.3.9 call-site bisect. **HARD self_compile + cap-headroom + dedup gates.** | — | ~5 |
| **v6.3.11** | **Async/await (G)** — `async fn`/`await` → CPS state machines over the epoll runtime (captures across await reuse E's env lowering) **+ native-float Tier A tail (H)** — aarch64 NEON + f32/conversions (f64 type+operators+NaN shipped .18/.19/.41). | x86-macOS usable-toolchain tail (ach-gated) | ~8 |
| **v6.3.12** | **[gate] W^X — `cyrld` separate code/rodata/data PT_LOAD (P1, linker)** — userland ELF emits a `R E` text segment + a `RW ` data segment (page-aligned) instead of one `RWE`, closing the last writable-executable surface on the agnos W^X loader (1.50.6 is already PF_X-aware → zero further kernel work). Reuses the `EMITELF64_KERNEL` multi-PT_LOAD machinery. **Changes every userland binary's layout → full release gate (seed-derive + cross-OS + `readelf -lW` on agnsh/kriya/owl).** [`proposals/2026-06-29-elf-wx-separate-code-data-segments.md`](proposals/2026-06-29-elf-wx-separate-code-data-segments.md). | — | ~6 |
| **v6.3.13** | **[gate] str_builder concurrent corruption root-cause (P1)** — 87% corruption under 8-thread load while a hand-rolled replica stays 100% clean → points at a compiler MISCOMPILE of specific `lib/str.cyr` fns, not a library logic bug. Investigation-first (bisect the miscompiled fn), then fix + a concurrency regression test. [`issues/2026-06-28-str-builder-not-thread-safe.md`](issues/2026-06-28-str-builder-not-thread-safe.md). | — | ~5 |
| **v6.3.14** | **[gate] var-decl codegen pair (P2)** — (a) `var p = mk()` INFERRED struct-local from a struct-returning call segfaults (retptr space not allocated; explicit `var p: T = mk()` works) [`inferred-struct issue`](issues/2026-06-28-inferred-struct-local-from-call-segfaults.md); (b) top-level `var X = "literal"` global init holds garbage — implement string-literal global init or hard-error at compile [`string-literal issue`](issues/2026-06-28-string-literal-global-initializer-garbage.md). Both are codegen fixes needing self-host + seed-derive. | — | ~5 |
| **v6.3.15** | **Closeout** — cross-feature integration tcyr (generic closures, async closures capturing a generic) + the minor-close pass (heap-map audit, dead-code, refactor, code-review, cleanup, doc/vidya sync). Ships code, not docs-only. | — | ~4 |
