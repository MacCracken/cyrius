...
> | **T** | `.28` | ⭐ **NEW — consumer-filed tooling/DX cluster.** Cheap, none touch codegen | (1) **`cyrius fmt --check`** — ⛔ **THE FILING'S HEADLINE IS REFUTED, and so was this roadmap's first draft of this row.** Wrapped calls are NOT rejected: cyrfmt PRESERVES the line break and normalises the continuation to the statement's own indent. A first pass here "reproduced" it with an 8-space continuation and reported *reproduced exactly as filed* — but that file genuinely differed from canonical form, so exit 1 was CORRECT. Reproducing *a* failure is not reproducing *the claim*. What IS real and worth fixing: `--check` exits 1 with **ZERO bytes** — no diff, no message, no line number — so any formatting difference is a silent CI failure. Fix the SILENCE; re-title the issue. (2) **`distlib` named-profile sidecar** (sit v1.4.0) — defect REAL and open, but the filing's title, its symptom 2 and its code hypothesis are all refuted by measurement (the author flagged the mechanism as unverified — correctly). Re-title before scheduling. (3) **`cyrius fuzz` blind to out-of-bounds READS** (sit v1.4.0) — ⭐ **EVERY premise verified, no correction needed; the best-formed of the three.** No sanitizer/redzone support exists anywhere, so overreads are structurally invisible: the bug class fuzzing is best at is half-covered **for every consumer in the ecosystem**. |
