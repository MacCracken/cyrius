...
| **v6.3.8** | **Lexical capture (E-capture)** — a closure body reading an enclosing local (a free variable) captured into a heap env at construction (by-value, no-GC-safe); the closure value becomes a (fn_ptr, env) pair / vtable-shaped object dispatched via `callptr(load64(obj), obj, args…)`. Free-var detection intercepts FINDLOCAL against a snapshot of the enclosing name-table (the v6.3.7 snapshot/restore is the foundation). The capture substrate async (G) reuses. **HARD: keep cycc self-host byte-identical (cycc uses no closures) + the closure post-pass-skip stays.** | — | ~7 |
| **v6.3.9** | **Real generic instantiation / monomorphization (F)** — type-param recognition + emit-time substitution + instantiation dedup, on Phase 0. **HARD self_compile delta budget as a release gate** (perf-refactor is v6.5.x — cost can't defer). | — | ~7 |
| **v6.3.10** | **Async/await (G)** — `async fn`/`await` → CPS state machines over the epoll runtime (captures across await reuse E's env lowering) **+ native-float Tier A tail (H)** — aarch64 NEON + f32/conversions (f64 type+operators+NaN shipped .18/.19/.41). | x86-macOS usable-toolchain tail (ach-gated) | ~8 |
| **v6.3.11** | **[gate] W^X — `cyrld` separate code/rodata/data PT_LOAD (P1, linker)** — userland ELF emits a `R E` text segment + a `RW ` data segment (page-aligned) instead of one `RWE`, closing the last writable-executable surface on the agnos W^X loader (1.50.6 is already PF_X-aware → zero further kernel work). Reuses the `EMITELF64_KERNEL` multi-PT_LOAD machinery. **Changes every userland binary's layout → full release gate (seed-derive + cross-OS + `readelf -lW` on agnsh/kriya/owl).** [`proposals/2026-06-29-elf-wx-separate-code-data-segments.md`](proposals/2026-06-29-elf-wx-separate-code-data-segments.md). | — | ~6 |
| **v6.3.12** | **[gate] str_builder concurrent corruption root-cause (P1)** — 87% corruption under 8-thread load while a hand-rolled replica stays 100% clean → points at a compiler MISCOMPILE of specific `lib/str.cyr` fns, not a library logic bug. Investigation-first (bisect the miscompiled fn), then fix + a concurrency regression test. [`issues/2026-06-28-str-builder-not-thread-safe.md`](issues/2026-06-28-str-builder-not-thread-safe.md). | — | ~5 |
| **v6.3.13** | **[gate] var-decl codegen pair (P2)** — (a) `var p = mk()` INFERRED struct-local from a struct-returning call segfaults (retptr space not allocated; explicit `var p: T = mk()` works) [`inferred-struct issue`](issues/2026-06-28-inferred-struct-local-from-call-segfaults.md); (b) top-level `var X = "literal"` global init holds garbage — implement string-literal global init or hard-error at compile [`string-literal issue`](issues/2026-06-28-string-literal-global-initializer-garbage.md). Both are codegen fixes needing self-host + seed-derive. | — | ~5 |
| **v6.3.14** | **Closeout** — cross-feature integration tcyr (generic closures, async closures capturing a generic) + the minor-close pass (heap-map audit, dead-code, refactor, code-review, cleanup, doc/vidya sync). Ships code, not docs-only. | — | ~4 |
