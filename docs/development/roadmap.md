...
| then | **Windows install + self-host** (verify/fix on cass via the REAL installer, not just self-host; build-windows job added .28; wire the cass arm of the audit cross-OS gate). PILLAR. | before AGNOS arc |
| then (behind Windows, user 2026-06-02) | **x86_64-macОS runtime arc** — the x86 Mach-O `cycc` **SIGSYS's on real compiles** (only partial Darwin BSD-ABI syscall xlat; arm64 got full `ESYSXLAT` in .34, x86 never did). A backend runtime arc mirroring arm64's .32–.34, THEN the packaging fix. Verifiable on `ach` (Intel Mac, SSH-wired). Issue `2026-06-02-macos-x86-release-no-compiler.md`. PILLAR. | before AGNOS arc |
