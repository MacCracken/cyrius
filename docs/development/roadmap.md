...
| **v6.1.35+** ⏳ | **Phase F continues** — F1b pt2 (CVE-23 emitter caps · CVE-31 frontend input validation · CO-02 check.sh exit-masking), then F2 (TLS-authn), F3 (memory-safety parity). See the Phase F detail below. | F — hardening |
...
| **F1b pt1** ✅ **v6.1.34** | `_vec_die`/`_hm_die` self-recursion → SIGSEGV (CVE-22, restore `syscall(60,1)`); DSE/LASE/#regalloc x86 byte-passes ungated on aarch64 (CO-03, gate `_AARCH64_BACKEND==0`) | [live-silent-failure-regressions](issues/2026-06-10-live-silent-failure-regressions.md) |
| **F1b pt2** ⏳ next | `output_buf` 16 MB cap unenforced on aarch64-ELF/PE/x86-kernel emitters — needs per-emitter PRE-write guards (CVE-23); frontend input validation — missing-include/unknown-char loud errors + `file_map` heap relocation (CVE-31); `check.sh` tcyr exit-masking (CO-02) | [live-silent-failure-regressions](issues/2026-06-10-live-silent-failure-regressions.md) |
