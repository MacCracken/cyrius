...
| **post-v6.2.33** ⏳ | **Delete `bootstrap/scaffold/bridge.cyr`** (the readable-cyrius proof-of-concept that de-risked the seed→cybs→cycc derivation above). **KEPT THROUGH the v6.2.33 tag DELIBERATELY** (user 2026-06-20): if the seed→cybs→cycc chain ever regresses, `git checkout v6.2.33 -- bootstrap/scaffold/bridge.cyr` restores the proven readable compiler to bisect cybs against — the recovery point we never want to lose. Delete in the FIRST slot AFTER .33 ships + is tagged. Dev-only, banner-marked, NOT in the trust chain (nothing includes it → removal is byte-identical to all artifacts). | cleanup / bootstrap |
