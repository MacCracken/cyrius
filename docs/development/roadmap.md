...
| ~.36 | **Windows install + self-host** (verify/fix on cass; build-windows job added .28; wire the cass arm of the audit cross-OS gate) | before AGNOS arc |
| ~.3x | **macОS `fn main()` return → exit propagation** — `fn main(){return N;}` exits 1 (not N) on Mach-O; the entry/exit epilogue drops `main`'s return before the BSD exit syscall. Pre-existing (NOT a .35 regression), masked because the tools `sys_exit` explicitly + the .33 smoke used `var x=42;`. Issue `2026-06-02-macho-main-return-exit-propagation.md`. Fix + add a real exit-code assertion to the macОS gate | before AGNOS arc |
