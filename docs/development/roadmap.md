...
| **v6.3.25** | **[consumer-filed bug — TLS concurrency] multi-worker TLS `RECORD_LAYER_FAILURE` under mixed load** — the last blocker to a multi-worker HTTPS server (`yeo-cy-test`, Medium). Clean at `max_conns=1` and under pure concurrency (100/100 simultaneous HTTPS POSTs), but a mixed sequential pattern — many fresh HTTPS conns including **one client that aborts mid-handshake (untrusted cert)**, interleaved with plaintext traffic — deterministically fails a later handshake with client-side `RECORD_LAYER_FAILURE` at `max_conns>1`. Points at per-connection TLS state (record seq / key schedule / arena reset after an aborted handshake) leaking into the next handshake a pooled worker serves. Root-cause across `lib/tls_native_*.cyr` + sandhi's `run_pooled_tls` per-conn arena reset (`src/server/mod.cyr` `_sandhi_server_pool_tls_worker`) — **likely cross-repo (sandhi/sigil)**; a `run_pooled_tls` gate that drives an aborted handshake then asserts the next N handshakes on the same pool succeed would lock it. [`multiworker-tls`](issues/2026-07-01-multiworker-tls-record-layer-failure-under-mixed-load.md). | — | ~4-5 |
| **v6.3.26** | **[v6.4.x pull-in — ABI] Class B FFI / `fncall6` ABI fix** — fix Cyrius's `fncall6` vs SysV AMD64 calling-convention bug (mabda's wgpu integration needs it). Held-forward through v5.9.x–v5.11.x. | — | ~5 |
| **v6.3.27** | **[v6.4.x pull-in — perf] Cross-BB regalloc + liveness** — linear-scan register allocator with cross-BB liveness data. **AR-04 substrate decision at entry**: extend the x86 byte-peephole vs. activate real IR-level register allocation (the latter is the only path to aarch64/riscv64 regalloc parity). Gated behind the differential corpus (v6.3.20) + bench un-blind (v6.3.17). | — | ~6 |
| **v6.3.28** | **[v6.4.x pull-in — perf] Copy propagation** — `ir_copyprop_recon` revival (deferred v5.6.18/.19; the stack-machine IR had no virtual registers for the classical wins — regalloc surfaces them). | — | ~3 |
| **v6.3.29** | **[v6.4.x pull-in — perf] Extended cross-BB DSE** — `ir_extdse_recon` revival (per-BB DSE shipped v5.6.18; the cross-BB variant needs the per-BB liveness-out set regalloc builds). | — | ~3 |
| **v6.3.30** | **[v6.4.x pull-in — perf] Float peephole + bench-delta eval** — `float.cyr:41` 5-instruction → 3-byte reduction (land if the bench delta justifies) + the ABI/perf-arc bench-delta evaluation + tcyr coverage (now measurable, given v6.3.17). | — | ~4 |
| **v6.3.31** | **[deps — modularity arc lever-1 COMPLETION] Phase D: dissolve the "stdlib" category + migrate the flagship** — the FINAL phase of the v6.2.x granularity arc (A/B/C shipped v6.2.48/.49/.50; **D was designed in `roadmap_6.md:456` but never had a slot until this audit — the whole arc is now scheduled, not 3/4 of it**). Reframe `[deps].stdlib` as a default group (`std = [...]`; `stdlib` stays a back-compat alias). Migrate **descent** off its hand-ordered stdlib list + whole-monolith includes as the acceptance proof (**verify descent's CURRENT manifest at slot entry** — the v6.2.x "29-element" count may have drifted). Bump the 256-file include-once cap (`src/frontend/lex_pp.cyr:271`) IF granularity splits push past it. **Acceptance:** cycc self-hosts byte-identical (CLI-only — `cbt/cyrius.cyr`, no cycc/backend change); pre-existing flat `stdlib`/`modules=` manifests resolve **byte-identical** (same back-compat bar as shipped lever-1/2); descent resolves the same symbol set with **no hand-ordered list + no SIGILL-on-omission**; one vidya entry per new surface. Full design: [`roadmap_6.md`](roadmap_6.md) §"Dependency model … Phase D". | — | ~3 |
| **v6.3.32** | **[language — generics arc TAIL] nested + multi-type-param + control-flow-in-body** — the three pieces v6.3.10 deferred out of its own cell with **no forward slot** (surfaced by this audit): **nested** `Outer<Inner<T>>`, **multiple distinct type params** `foo<T, U>`, and **control-flow** (if/while/match) inside generic-fn bodies, on the instantiate-once engine. The first ceilings real consumers hit after adopting generics. Gated proof like the rest of the arc (`_MONOMORPH_OK`), byte-identical flag-off; x86 + aarch64 exit-correct + differential + seed→cybs→cycc. | — | ~4-5 |
| **v6.3.33** | **[stdlib — consumer-filed, hoosh] `lib/protobuf.cyr` proto3 wire codec** — **user-granted its OWN slot 2026-06-16** (hoosh OTLP/OpenTelemetry span export); dropped from the table until this audit. Minimal proto3 wire encode/decode — varints, length-delimited messages, no `.proto` compiler; pure Cyrius, no syscalls. Lib-only → cycc byte-identical (like the .6/.14/.18/.19 stdlib pull-ins). hoosh ships OTLP/JSON today (works) so not hard-blocking — the gap was the missing slot. | — | ~2-3 |
| **v6.3.34** | **[Intel-Mac arc, tail] x86-macho native miscompile fix** — the core blocker (**High**): the cross-built cycc emits a broken NATIVE x86_64 Mach-O cycc (`backend/x86/emit.cyr` + `backend/macho/emit.cyr`) — Intel Macs have no working compiler. [`macos-x86-release-no-compiler`](issues/2026-06-02-macos-x86-release-no-compiler.md). | — | ~3 |
