...
| **v6.1.40+** ⏳ | **Phase F tail** — F1+F2 done, F3 5/6 shipped + diagnostic audit (.39). **Next: CVE-24 grow-and-cap** — fragile 75+-ref heap relocation of the 4 slot-indexed local tables to a fresh 256 KB block + cap ([slot-indexed-overflow](issues/2026-06-12-locals-table-slot-indexed-overflow.md)). Then the dep-fold cycle-close → v6.2.0; bug bandwidth as filed. | F → v6.2.0 |
...
| **F3 — memory-safety parity** ✅ **v6.1.38** (5/6) | `_sb_grow` OOM (CVE-25), `alloc_agnos` size guard (CVE-26), PE import-registry bounds (CVE-27), aarch64 atomics `ldaxr`/`stlxr` barriers + vtable fence (CVE-28), fixup cap unify 262144→1048576 + `jump_target_tbl` off-by-one all-4-accessors (AR-03). Adversarial review caught the jump fix touching only 1 of 4. **CVE-24 deferred** (locals table is slot-indexed → naive cap breaks large `stack var` frames). | [archived](issues/archived/2026-06-10-memory-safety-parity-gaps.md) · CVE-24 re-scoped → [slot-indexed-overflow](issues/2026-06-12-locals-table-slot-indexed-overflow.md) · CVE-28 from [unreviewed-dimensions](issues/2026-06-10-unreviewed-dimensions.md) (LEGAL-01/CVE-29/DX open) |
