...
| **v6.1.34+** ⏳ | **Phase F continues** — F1b live-silent-failures (CVE-22/23/31, CO-02/03), then F2 (TLS-authn), F3 (memory-safety parity). See the Phase F detail below. | F — hardening |
...
| **F1b — silent-failure** ⏳ next | `_vec_die`/`_hm_die` recursion (CVE-22), `output_buf` cap unenforced on aarch64/PE/kernel (CVE-23), silent-bad-input (CVE-31), `check.sh` exit-masking (CO-02), x86-opt passes unguarded on aarch64/cx (CO-03) | [live-silent-failure-regressions](issues/2026-06-10-live-silent-failure-regressions.md) |
