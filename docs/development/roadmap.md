...
| next (user order 2,1,3) | **macОS `fn main()` return → exit propagation** — `fn main(){return N;}` exits 1 (not N) on Mach-O; entry/exit epilogue drops `main`'s return before the BSD exit syscall. Pre-existing (NOT a .35 regression). Issue `2026-06-02-macho-main-return-exit-propagation.md`. Fix + add a real exit-code assertion to the macОS gate | before AGNOS arc |
