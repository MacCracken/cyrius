...
| .90 | **cleanup / refactor cluster (part 2b) — `programs/check.cyr` modularization** (10.2K LoC / 163 fns → `programs/checks/main.cyr` dispatcher + ~11–12 suite files). The understand-phase workflow produced the full breakdown (infra/shared/selfhost/crosshost + platform_efi/win_macho/aarch64 + cx/ts/deps_init/lint_fmt/heap_audit/services) and a behavioral-identity acceptance plan (`_run_regression_gates`+`main`+18 globals stay VERBATIM; suites are fn-bodies only; same per-gate output + `N passed/M failed` + tcyr exit loop; NOT byte-identical since `cyrius_check` isn't in the self-host chain). **One open ASK:** split the oversized `services.cyr` (~1930 L) into `services.cyr`+`codegen_regress.cyr` (13 files). **Latent to fold:** macho-arm socket *family* (socket/connect/accept) mapped with x86 nums in the aarch64-macho ESYSXLAT but arm-macho uses aarch64-Linux nums → real-network sockets likely broken on arm-macho (cf. sandhi-connect issue). | |
| .91 | **closeout + docs sweep** — CLAUDE.md Closeout Pass § (mechanical fail-fast → judgment passes → doc sync; reconcile the slot-number drift this long cycle accreted) | |
