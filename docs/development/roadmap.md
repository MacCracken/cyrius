...
| **v6.3.15** | **[gate] array-locals default-on (P1, completes .13)** — flip per-thread array locals from opt-in to DEFAULT. Blocker found at .13's default-on flip: **SSE m128 16-byte alignment** — inline-asm AES-NI / crypto / TLS use arrays as `xmm` operands (`pxor xmm,[arr]`) needing 16-aligned addresses; global arrays got it via the v5.5.21 totvar pad, but stack slots are only 8-aligned → `#GP`. Fix = **16-align array-local stack slots** (pad the frame so the array base `[rbp-disp]` is ≡ 0 mod 16) + root-cause the TLS-probe compile failure + re-flip default-on (two-step bootstrap; the m128 regression suite + multi-worker-TLS round-trip are the canaries). Changes every binary with an array local → full release gate. [`array-locals arc`](issues/2026-06-30-array-locals-stack-default-on-m128-align.md). | — | ~5 |
| **v6.3.16** | **[gate] var-decl codegen pair (P2)** — (a) `var p = mk()` INFERRED struct-local from a struct-returning call segfaults (retptr space not allocated; explicit `var p: T = mk()` works) [`inferred-struct issue`](issues/2026-06-28-inferred-struct-local-from-call-segfaults.md); (b) top-level `var X = "literal"` global init holds garbage — implement string-literal global init or hard-error at compile [`string-literal issue`](issues/2026-06-28-string-literal-global-initializer-garbage.md). Fold the **1-field-struct segfault (P2)** [`issue`](issues/2026-06-30-single-field-struct-segfaults.md) here (same struct-local-codegen family). Both/all are codegen fixes needing self-host + seed-derive. | — | ~6 |
| **v6.3.17** | **Closeout** — cross-feature integration tcyr (generic closures, async closures capturing a generic, concurrent array-locals) + the minor-close pass (heap-map audit, dead-code, refactor, code-review, cleanup, doc/vidya sync). Sweep the residual live issues (tls13-server-get-version-zero P3, bare-metal-forbidden-module-check #4 P3, agnos-syscall-peer 8 wrappers, syscall-write-byte-length-gate, bare-local-array-slot-write-lint) into bug-bandwidth or pin to v6.4.x. Ships code, not docs-only. | — | ~4 |
