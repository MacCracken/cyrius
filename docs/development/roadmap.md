...
| **v6.2.28** ⏳ | **Bare-metal target formalization — RUNTIME HALF** (user-pinned 2026-06-19). The runtime/validation surface: **D5 kernel-freestanding TLS** — the one TLS gap is ENTROPY (the v6.2.4 transport vtable abstracts read/write/now but NOT randomness): add a 4th vtable slot `_tn_tx_rand` + a `_tn_rand_bytes` leaf funneling all 12 raw `sys_getrandom` sites (hs13×9 / hs12×3), so the kernel plugs entropy + alloc (the arena handle already threads via `tls_native_new_server_in`); **D6** expose `_entry_base` VA via the triple (minimal — not full `#section` syntax); **D7 boot acceptance gate** — a REAL QEMU boot of the kernel built for the triple (NOT a compile-only ELF-shape placebo — the macho-rot lesson), porting the agnos boot harness into cyrius CI. D7 anti-rot-gates .27's codegen. **+ `#naked` follow-on** (carried from .27 — `2026-06-19-naked-fn-safety-and-inline-asm.md`): the param/return safety guards don't fire on DCE-stubbed naked fns (make DCE never stub a `#naked` fn), and the **inline-asm path** (`asm(...)` or `emit_iretq`/`emit_eret`/… intrinsics) that real ISRs need — both first exercised here by the boot gate + the AGNOS IDT. | bare-metal runtime |
| **v6.2.29** ⏳ | **VR-01/VR-02 verification fold** (was .28; pushed by the bare-metal arc split) — close the verification gaps the .25 Windows break exposed: full-`.tcyr` loop into the **arm-native CI job**, **`cyrius fuzz` → a check.sh gate**, + a **CLI cross-compile check** (`CYRIUS_TARGET_WIN`/`_MACOS`/`_AARCH64` on `cbt/cyrius.cyr` — the exact class the .25 break slipped through). From `2026-06-10-verification-coverage-gaps`. | verification/CI |
| **v6.2.30** ⏳ | **CVE-21 release / trust-chain integrity** (was .29) — seed-disjoint trust root + fail-closed release path (`install.sh:373` still "continuing anyway" on a bad checksum). From `2026-06-10-release-trust-chain-integrity`. | security |
| **v6.2.x** ⏳ | **Dependency model — modules + module groupings (stdlib → standard deps)** (user-added 2026-06-19; before RISC-V). Evolve the flat `[deps] stdlib = [list]` toward first-class **standard deps** where a dep can be a single **module** (e.g. `json`, `sha256`) OR a declared **module grouping** (e.g. a `crypto` group bundling sha/keccak/ed25519 — making the de-facto bayan/ganita/sandhi/sigil bundles an *explicit* grouping mechanism rather than hand-vendored monoliths). Goal: dissolve the special "stdlib" category — the ecosystem libs become ordinary deps with module granularity, so a consumer pulls exactly the modules it needs (smaller preprocess footprint — cf. the mabda opt-in 515 KB cap note) instead of all-or-vendored. The granularity + grouping foundation UNDER the v6.3.x required-vs-optional-deps (Cargo-style features + platform-conditional) item — land the module/grouping model here, the feature-flags/optional layer at v6.3.x. | deps/packaging |
