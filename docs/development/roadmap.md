...
| .88 | **cleanup / refactor cluster** — dead `ret_patches` heap region, byte-array literal peephole, dead-code sweep, `_TARGET_*` flag consolidation, `programs/check.cyr` modularization, sigil refold if current tag moved. **Latent to fold:** macho-arm socket *family* (socket/connect/accept) mapped with x86 nums in the aarch64-macho ESYSXLAT but arm-macho uses aarch64-Linux nums → real-network sockets likely broken on arm-macho (cf. sandhi-connect issue). | |
| .89 | **closeout + docs sweep** — CLAUDE.md Closeout Pass § (mechanical fail-fast → judgment passes → doc sync; reconcile the slot-number drift this long cycle accreted) | |
