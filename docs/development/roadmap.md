...
| **v6.2.x** ⏳ | **Bare-metal target formalization** — codify the ad-hoc bare-metal mode (agnos has used since first boot) into a first-class `--target <arch>-bare-metal-elf` triple. 7 deliverables: target triple, ELF no-libc output (no PT_INTERP/DT_NEEDED/libc-init `_start`), interrupt-handler emit conventions (`naked_fn`), no-runtime-assumptions codegen, **kernel-freestanding TLS linkage** (the AGNOS-kernel crypto prereq), linker-script/section control, and the cross-OS + agnos boot acceptance gate. Bottom-to-top priority — lands before RISC-V. | bare-metal |
