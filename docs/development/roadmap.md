...
| .87 | **AGNOS — cross-build gate + envp/getenv** (user opt #2). `cyrius build --agnos` already emits a valid agnos ELF; add the arc-.32 cross-build gate (rot-proofing) + the cyrius envp/getenv-on-agnos side. **Gated on agnos landing envp-at-exec first** (agnos passes empty envp today; agnos 1.43.x) — "need envp/getenv done in agnos first" (user). No cross-repo agnos edit. | |
