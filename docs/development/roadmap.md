...
| **v6.2.31** ⏳ | **CVE-20/21 trust-chain integrity — PART 2: sovereign signing + reproducibility attestation** (user 2026-06-19/20). (a) **Sovereign** detached signing — `cyrsign` (a standalone tool over sigil's in-tree ed25519: keypair/sign/verify) signs `SHA256SUMS`; pubkey committed in-repo + SECURITY.md; `install.sh`/`ci.sh`/`install.ps1` verify with the cyrius verifier on the upgrade/CI path (no external `minisign` — sovereignty). Closes CVE-13. (b) **Reproducibility-attestation CI** (NOT the literal seed→cycc — see the bridge arc below): formalize `build-cycc-verify.sh`'s self-host fixpoint + the per-OS reproduce-itself jobs into a trust-root-attest job = diverse double compilation (pi/ecb/cass each reproduce `cycc` byte-identical). This is the **interim** CVE-20 mitigation — proves the committed `cycc` is reproducible-from-source; the real machine-*derivability* is the bridge arc. | security |
| **v6.2.x** ⏳ | **🔴 Seed→cycc bridge restoration — the REAL CVE-20 fix (MAJOR; user-prioritized 2026-06-20, BEFORE the deps-modules item).** Today `seed → cybs → asm` is real (assembly-level, closure-verified) but **`cybs → cycc` does not exist** — `cybs` is an assembler, can't parse `src/main.cyr` ("syntax error"); the de-facto trust root is the committed `build/cycc` blob nobody re-derives (CVE-20). Restore the retired ladder rung (`src/bridge.cyr`-class, ~2000 LoC, dropped v5.11.66): a minimal cyrius compiler that **cybs/asm can build AND that can compile modern `src/main.cyr`** (directly or via an intermediate), making the literal **`seed → cybs → bridge → cycc`** chain executable + machine-checkable — i.e. "the way we did it before." Supersedes the .31 attestation interim. Multi-slot arc; gate = a clean machine derives `cycc` byte-identical from the 29 KB seed + source. | security / bootstrap |
