...
| 4 | **DX diagnostics R1 ✅ COMPLETE v6.4.60** — column + source-excerpt with a caret on every error (packed token offset into `tok_lines`; all ~452 sites via one shared emitter; `at fail:` dumps retired). **R2 = bounded multi-error** (statement/decl panic-mode; the dead `_had_error` hook is ready; riskier — desync crash exposure) is filed as the follow-up. | ~~column/source-excerpt~~ ✅ .60; `2026-07-12-dx-multi-error-reporting` (R2, P2) | P2 |
