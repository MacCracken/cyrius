...
| .34 | **macOS tools — cyrfmt/cyrlint/cyrdoc** (broad macho-arm BSD-ABI expansion: ESYSXLAT full syscall surface + openat/at-family reroutes + fork x1-fixup + macOS OpenFlag + 256MB heap reserve + wrapper arch/HOME) | ✅ COMPLETE — tools run on ecb; `cyrius build` wrapper deferred to .35 |
| .35 | **`cyrius build` macОS SIGBUS — FIXED.** Root cause was NOT a fixup bug: `&IDENT` checked `FINDFN` before `FINDLOCAL`/`FINDVAR`, so a local/global shadowed by a same-named function (wrapper `var argv` vs `lib/args.cyr` `fn argv`) resolved to the function's `__TEXT` address; the store faulted under macОS W^X (masked on Linux's RWE segment). Fix = resolve `&` in local→global→function order. Verified on ecb: cycc self-host byte-identical + `cyrius build` runs | ✅ COMPLETE — proven on ecb |
| **platform-cleanup window (before AGNOS binary, user 2026-06-02)** | | |
| .36 ✅ COMPLETE | **Open-issue LHF cleanup batch** (bited commits, user 2026-06-02). 7 bites + 2 verified-resolved + 1 won't-fix. Fixed: `u256_addmod` 2²⁵⁶-carry (`lib/bigint.cyr`) + regression test; `cyrlint` conventional exit 0/1/2 + `--strict`/`--exit-with-count`; `cyrius build` unknown-flag + `.cyr`-output clobber guards; `cbt` `modules` boundary/comment-aware (all 3 readers); `cyriusly` starship symlink-safe + `.bak`; build-artifact **pre-commit hook** + `cyrius hooks install` + install.sh auto-install. Archived already-resolved: `type-table-256-cap` (diagnostic ships), `distlib-blank-lines` (fixed v6.0.9). Closed won't-fix: `for-empty-clauses` (documented). No `src/` change → self-host byte-identical; 82/82. | ✅ shipped |
| follow-up | **secret/defer per-fn cap 8→32** (own release — heap-map layout change, P4) | before AGNOS arc |
| .37 ✅ COMPLETE | **macОS `fn main()` return → exit propagation** — `main_aarch64_macho.cyr` (stale fork) never got `main_aarch64.cyr`'s v5.9.37 auto-call-`main` fix → the Mach-O entry stub skipped `bl main` and exited with `argc` (1). Ported the fix (walk fn table for `main\0` → `ECALLTO`); native-compiler-only (cross-emit was fine). Added an exit-code assertion to the `cyrius audit` macОS gate (`fn main(){return 42;}`→42). Verified on ecb: self-host byte-identical + exit-code propagation; x86 unchanged. | ✅ shipped |
| .38 ✅ COMPLETE | **macОS arm64 INSTALL ships a real compiler.** Release `build-macos-arm64` shipped only a `smoke.macho` toy → every Apple Silicon install was EMPTY (the `.32` problem #1, punted 5 releases + masked by SSH cross-builds). Now ships the full toolchain in `bin/` via `scripts/build-macos-arm64-tarball.sh` (single source of truth); `install.sh` codesigns + hard-fails on no-bin + local-tarball override; `cyrius build` ad-hoc-signs output; **`cyrius audit` real-install gate** (build→install.sh→`cyrius build`→42). Verified via the REAL installer on ecb. | ✅ shipped |
| **PILLAR RULE (user 2026-06-02)** | Each platform's REAL `install.sh` must yield a working toolchain, verified on hardware — "we don't support a platform if the installer doesn't work." Self-host ≠ install. | |
| .39 ✅ (partial — arc opened) | **Windows `cycc` runtime — bug 1 fixed.** Dug on real `cass`: `cycc` has NEVER run as a compiler on Windows (crashed on startup; hidden ~30 minors because CI only ran EMITTED PE programs). Fixed `alloc_windows.cyr` `syscall(12)`→`syscall(9)` (brk reroute never existed; `vec_new` crashed on first alloc) + pe HIGH_ENTROPY_VA hygiene. cycc now runs + reads stdin on cass, but still generates 0 code (bug 2: `BL` lost before parse / `SBL`-store). **Windows = a multi-slot RUNTIME ARC, not a one-line item.** Issue `2026-06-02-windows-cycc-runtime-multibug.md`. | bug 1 shipped; arc open |
| **NEXT (user 2026-06-02): x86_64-macОS runtime arc** | the x86 Mach-O `cycc` **SIGSYS's on real compiles** (only partial Darwin BSD-ABI syscall xlat; arm64 got full `ESYSXLAT` in .34, x86 never did). Backend runtime arc mirroring arm64's .32–.34, THEN packaging. Verifiable on `ach` (Intel Mac, SSH-wired). Issue `2026-06-02-macos-x86-release-no-compiler.md`. PILLAR. | before AGNOS arc |
| then (return to grinding) | **Windows `cycc` runtime arc — bug 2+** (root-cause `SBL`/`GBL` store → fix compile→emit→write path → auto-call-main → self-host on cass → REAL install → cass audit gate). PILLAR. Issue `2026-06-02-windows-cycc-runtime-multibug.md` | before AGNOS arc |
...
| .85 | **Windows install pillar** — `cyrius build` works on Windows (macOS-`.38` bar). GetEnvironmentVariableA PE reroute (`0xF015`) → wrapper env/`.exe` cycc resolution → `compile()` cmd.exe-redirect spawn via CreateProcessW → native `install.ps1` + `build-windows-tarball.sh` (SSOT, ships `cyrius.exe`) → cass `cyrius audit` install gate → `cyrius deps --lock` certutil hash. PE cycc-runtime "bug 2" found ALREADY-resolved (stale pin). | ✅ COMPLETE — cass install gate green (install→build→42); ecb+ach+pi self-host |
| .86 | **last of Windows — DXGI demonstrator** ✅ COMPLETE. Root cause (cdb on cass): the `callptr` callee-spill frame slot could ALIAS a `&`-taken local (EnumAdapters1's `&pAdapter` out-param == the callee-spill slot) → frame/regalloc corruption → COM-dispatch AV. Fixed by emitting the PE callptr callee on the stack + `call rax` (no frame slot). `dxgi_vram_bytes()` GPU-verified on cass (UHD 600 → 128 MB); ai-hwaccel unblocked. **Windows arc fully closed.** | ✅ COMPLETE |
| .87 | **AGNOS — cross-build gate + envp/getenv** (user opt #2) ✅ COMPLETE. `getenv()` agnos branch walks the exec-stack envp from `_agnos_init_rsp` (ABI §4.6); arc-.32 cross-build gate (`scripts/agnos-crossbuild-gate.sh` + `test-agnos` CI) compiles a probe + agnoshi → valid agnos ELF. **Verified on the REAL agnos 1.43.2 kernel under QEMU** (`getenv("HOME")=/`, `PWD=/`, unset→null). No cross-repo agnos edit. | ✅ COMPLETE |
| .88 | **cleanup / refactor cluster (part 1)** ✅ COMPLETE — sigil 3.7.4→3.7.7 + sandhi 1.4.2→1.4.4 refold; dead `ret_patches` 2 KB heap region removed (`[0x18DA20..0x18E220)` FREED, dead init writes gone, −48 B); **byte-array literal peephole** (`_EVRCX`-hoisted base + `ESTOREB_IMM` direct store, x86 disp8/disp32 + aarch64 `strb`-imm + cx; ~5× compression, obj-mode-correct); dead-code sweep (removed legacy `EMIT_STRUCT_FIELD` wrapper; floor 69→68 fns / 26 818 B, rest scaffold). Self-host byte-identical on **ach+ecb+pi+cass**; check.sh 85/85; tcyr 167/167. **Cut early (user 2026-06-07)** — the two heavy refactors split to .89/.90. **Found (pre-existing, filed `2026-06-07-x86-macho-byte-array-literal-no-compile.md`):** x86-macOS cycc can't compile byte-array literals (.87 fails identically; invisible to self-host). | ✅ COMPLETE |
| .89 | **cleanup / refactor cluster (part 2a) — `_TARGET_*` consolidation, FIRST BITE** ✅ COMPLETE. Leader picked this over check.cyr for .89. The format-selection ladders duplicated *within* each fixup backend (entry-VA in `FIXUP` + dispatch in `EMITELF`) collapse into a shared **`_emit_fmt`** selector (0=ELF/1=Mach-O-x86/2=Mach-O-ARM/3=PE) + **`_entry_base`** helper, in x86+aarch64 fixup. Deliberately narrow — the rest is genuinely multi-slot (understand-workflow confirmed: ~76 byte-incompatible per-op emitter branches + ~20 load-bearing shims). `_emit_fmt` tests Mach-O before PE (matches old precedence for the unguarded `CYRIUS_MACHO`+`CYRIUS_TARGET_WIN` combo). **Byte-identical across ALL modes** (ELF/kernel32+64/shared/obj/EFI/AGNOS/PE/macho-x86/macho-arm + the macho+PE conflict) via .88-vs-.89 binary diff; self-host on ach+ecb+pi+cass; check.sh 85/85. A 3-lens adversarial-review workflow caught+fixed a draft PE/Mach-O precedence flip. | ✅ COMPLETE |
| .90 | **cleanup / refactor cluster (part 2b) — `programs/check.cyr` modularization** ✅ COMPLETE. 10.2K LoC / 163 fns → `programs/checks/main.cyr` dispatcher (19 includes + 18 globals + 13 infra/driver fns VERBATIM) + **13 suite files** (shared/selfhost/crosshost + platform_efi/win_macho/aarch64 + cx/ts/deps_init/lint_fmt/heap_audit/services/codegen_regress — leader chose the 14-file layout, splitting codegen-behavior gates off so the largest suite is ~1.3K L). Deterministic verbatim fn-relocation (all 163 bodies bit-identical); rebuilt `cyrius_check` output **byte-identical** to the monolith (605 L, 85/85). `check.sh` repointed + globs all suite files for staleness; old monolith removed; refs in cbt/deps + cyaudit + process.cyr + ci.yml repointed. 2-lens adversarial review clean. **Closes the .88–.90 cleanup/refactor cluster.** | ✅ COMPLETE |
| .91 | **closeout + docs sweep** ✅ COMPLETE — CLAUDE.md Closeout Pass §. Mechanical all green (self-host + bootstrap closure + check.sh 85/85 + cross-OS ach/ecb/pi/cass); 6-agent judgment workflow clean (heap-map FREED region verified dead; dead-code floor 68 fns; security clean; 76 downstream pins zero anomalies). Residual `programs/check.cyr` comment refs swept (lib/fs/regression/audit_walk + probe programs + 1 tcyr). Vidya .88–.90 gotchas added. **Found (pre-existing, filed):** aarch64 `EADDRA_IMM` 12-bit mask → byte-array literals >4096 corrupt (not a regression; deferred v6.1.x). **v6.1.x carry-in** (roadmap-future): `_emit_fmt`/`_entry_base` shared-hoist (confirmed blocked by single-pass parser + include order — validates the .89 per-backend call), DCE mark-and-sweep consolidation. **Latent to fold:** macho-arm socket *family* (socket/connect/accept) mapped with x86 nums in the aarch64-macho ESYSXLAT but arm-macho uses aarch64-Linux nums → real-network sockets likely broken on arm-macho (cf. sandhi-connect issue). | ✅ COMPLETE |
| 6.1.0 | **clean cycle open + docs update** | |
