...
| **v6.2.27** ⏳ | **Bare-metal target formalization — FRONTEND HALF** (user-pinned 2026-06-19; split contiguous .27+.28 after the premise-check showed a 2-release arc). The CLI/frontend surface: **D1** a first-class `--target <arch>-bare-metal-elf` triple (a named front-end over the existing kmode/`kernel;` + `CYRIUS_ELF64_KERNEL` + `set_arch` — glue not codegen; env-default-0 keeps cycc byte-identical; mirror across the 6 `main_*` forks); **D2/D4** verify the no-libc freestanding ELF (`EMITELF{,64}_KERNEL` already omit PT_INTERP/DT_NEEDED/libc-`_start`) is reachable + clean under the triple (+ aarch64 kernel-emit parity); **D3 `naked_fn`** attribute (skip prologue/epilogue/frame for ISRs — rides the `#pure` attribute rail; MANDATORY 3-fork token mirror per the annotation-fork-desync class + the `frame_patch` dangling guard). Self-host-provable on x86 + aarch64. | bare-metal frontend |
| **v6.2.28** ⏳ | **Bare-metal target formalization — RUNTIME HALF** (user-pinned 2026-06-19). The runtime/validation surface: **D5 kernel-freestanding TLS** — the one TLS gap is ENTROPY (the v6.2.4 transport vtable abstracts read/write/now but NOT randomness): add a 4th vtable slot `_tn_tx_rand` + a `_tn_rand_bytes` leaf funneling all 12 raw `sys_getrandom` sites (hs13×9 / hs12×3), so the kernel plugs entropy + alloc (the arena handle already threads via `tls_native_new_server_in`); **D6** expose `_entry_base` VA via the triple (minimal — not full `#section` syntax); **D7 boot acceptance gate** — a REAL QEMU boot of the kernel built for the triple (NOT a compile-only ELF-shape placebo — the macho-rot lesson), porting the agnos boot harness into cyrius CI. D7 anti-rot-gates .27's codegen. | bare-metal runtime |
