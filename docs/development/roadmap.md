...
| **v6.2.4** ⏳ | **TLS transport-vtable refactor (Option C)** — the principled cleanup deferred from 6.2.3: give `tls_native`'s ctx fn-pointer plug points (read/write/now) so transport is swappable, instead of the v6.2.3 tagged-fd routing inside the agnos `sys_read`/`write`/`close`. Refactors the ~6 `_tn_sock_*` I/O sites to call through the vtable; defaults bind to `sys_read`/`sys_write`/`_tn_now_unix` (Linux byte-identical). **Dovetails with the bare-metal slot's "kernel-freestanding TLS" requirement** — the AGNOS kernel needs exactly this swappable transport to plug its own send/recv, so doing it once here serves both. Logic-preserving → byte-identical self-host on all hosts is the gate. | agnos-target / refactor |
