...
| bench (every-release gate) | self_compile **516 ms** @ 6.2.47 (bench-history.sh; within the 500–549 ms jitter band — .47 is CLI-only + cycc byte-identical, so the +11 ms vs .46's 505 ms is pure measurement jitter, not a delta); x86 cycc **1,073,672 B** (unchanged @ .47; +112 B @ .45) |
