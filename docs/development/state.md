...
| bench (every-release gate) | self_compile **511 ms** @ 6.2.49 (bench-history.sh; within the 500–549 ms jitter band — .46–.49 are all CLI-only + cycc byte-identical, so the variation is pure measurement jitter, not a delta); x86 cycc **1,073,672 B** (unchanged @ .46–.49; +112 B @ .45) |
