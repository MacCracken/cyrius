...
| **cycc_aarch64** (x86-host cross, emits aarch64) | **625,824 B** (rebuilt @ 6.2.44 version-bump — +144 B from the 2 aarch64 `_PE` stubs (EPROCPRNG_PE/EGETSYSTIME_PE); pi SELFHOST_OK) |
...
| **cycc_win** (PE32+ cross) | **850,432 B** (rebuilt @ 6.2.43 version-bump — grew from the .41 shared frontend/backend changes; cass SELFHOST_OK) |
...
| stdlib | **98** `lib/*.cyr` (−1 @.37 — `agnosys.cyr` retired) · 79 programs · api-surface **4344 fns** (+1 @.44 — `main_aarch64_native::EMITMACHO_ARM64/1` native-fork stub; +9 @.39 — agnos peer `sigset_new/add/has` + `sys_net_config` + 4 net getters + `sys_winsize`; all non-breaking, 0 removals) |
...
| bench (every-release gate) | self_compile **513 ms** @ 6.2.44 (bench-history.sh; cycc byte-identical to .41–.43 modulo the version string — x86 `src/` untouched, so self_compile measures the same binary; single-run history band 500–549 ms jitter); x86 cycc **1,073,560 B** (unchanged) |
