...
compiler-logic change** (cycc self-hosts byte-identical; only the `--version` string moves,
931,872 → **931,864 B**). check.sh to run as the final gate; self-host verified byte-identical.

**Benchmark (new every-release gate — CLAUDE.md Release rule #6):** `self_compile` **463 ms**,
`cycc` **931,864 B**. `BENCHMARKS.md` + `bench-history.csv` were stale since ~2026-05-18; refreshed.
vs the 404 ms (v5.11.63) baseline that's +59 ms over the whole ~91-patch v6.0.x minor ≈ +0.65 ms/patch
— growth-tax, not a regression.

**Roadmap split into three tiers:**
- `roadmap.md` → **active minor only** (v6.1.x), slot-by-slot.
- **`roadmap_6.md`** (new) → whole-v6.x-cycle reference (framing, budgeting, v6.2.x → v6.5.x, +
  a one-screen v6.0.x-COMPLETE summary; the 90+ per-item v6.0.x entries removed — canonical detail
  is CHANGELOG + completed-phases.md).
- `roadmap-future.md` → beyond-v6.x (unchanged). Pointers updated in CLAUDE.md + roadmap-future.md.
- Reading order: roadmap.md → roadmap_6.md → roadmap-future.md.

**`build/` prior-major slot corrected `cc3 → cc5`** (user 2026-06-08). "cc3 is dead" — confirmed
(v4.x-era, never in the bootstrap chain `bootstrap/asm → cybs → cycc`, nothing in build/CI/install
loaded it). The slot SHOULD have rotated cc3 → cc5 at the v6.0.0 cut but didn't, leaving cc3 a stale
prior-*prior*. Fixed: cc3 removed from git + `.gitignore` (`benches/bench_capacity_overhead.sh`
repointed to `cycc`); the real previous-major **`cc5` (5.11.69, 874,232 B) recovered from git history**
(`git show 4c531d12^:build/cc5`) and tracked. **`build/` now tracks `cycc` + `cc5` +
`cycc-native-aarch64`** (the last = the one cross-bin that can't be cross-built to run natively on ARM).
`cycc_aarch64` / `cycc_win` stay built-on-demand (user: "keep building on demand"). Stale pre-rename
`cc5_*` scratch swept from `build/`. The "current + one prior-major" policy is intact, just pointed at
the correct prior-major.

**Benchmark is now an every-release gate** (user 2026-06-08: "include benchmarks as part of every
release cycle... only occasionally / CI-only is disturbing") — CLAUDE.md Release & Slot Discipline
rule #6; [[feedback_benchmark_every_release]].

**Docs sweep (full):** fixed the false "everything is i64 / no floats" framing (tutorial / guide / faq),
added octal-literal docs + corrected the AGNOS include block + a wrong `var buf[256]`="2048 B" claim in
the guide, repointed 4 dead `regression-*.sh` refs in architecture/cyrius.md, added a UEFI row to
platform-status.md, banner-marked migration-strategy.md as a frozen v5.7.39 historical snapshot, and
bumped version/size markers (README / size-comparisons / platform-status / faq) to v6.1.0.
**Flagged for human follow-up:** stdlib-reference.md covers only ~33/90 lib modules (native TLS, SIMD/
matrix/linalg, async/thread/atomic, base64/bigint/u128, sha1/keccak, cffi, ws, folded deps undocumented).

**v6.1.x first-work** (per [roadmap.md](roadmap.md)): PIE codegen (x86 → aarch64) + `.gnu.hash` migration
+ back-compat symlink drop; carry-ins = bayan/ganita carve, EADDRA_IMM >4095 fix, `_emit_fmt` hoist,
DCE consolidation, POSIX `*at()`. **Cross-OS reverify (ecb/cass/pi) required** before any compiler-emit
slot cuts.
