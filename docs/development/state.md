...
| tests | **193** `.tcyr` (+`inline_asm_fences` @6.3.3 — EXECUTES mfence/lfence/sfence (x86) / dmb/dsb/isb (aarch64), real unprivileged-encoding proof, arch-conditional; +`float_nan_compare` @.41 — IEEE-754 unordered semantics: builtins/`f64_le`/`f64_ge`/operators on NaN/±Inf + ordered regression, 41 assertions; +`assert_fatal` @.38) · 15 `.bcyr` · 5 `.fcyr` |
...
| bench (every-release gate) | self_compile **504 ms** @ 6.3.3 (bench-history.sh; flat vs 505 ms @6.3.2 — the fence/base-accessor code isn't hot); x86 cycc **1,077,136 B** @6.3.3; self_compile **505 ms** @ 6.3.2 (within the 500–549 ms jitter band; no regression from the B1 fixup-gate flip — undef-check isn't hot); x86 cycc **1,075,616 B** @6.3.2 (was 508 ms / 1,075,136 B @6.3.1; v6.3.1 was cbt-only so cycc is untouched — no codegen delta); x86 cycc **1,075,136 B** (unchanged @6.3.1 — cbt-only; the size set @6.3.0 var-family migration, was 1,073,672 B @ 6.2.46–.52) |

> **Handoff (2026-06-28):** **v6.3.3 CUT — bare-metal #5 (settable kernel load base via `[sections]`)
> + #6 (x86 inline-asm fences).** Premise-check found the roadmap STALE on #6 (only `mfence`/`lfence`/
> `sfence` were missing — the rest shipped .27/.28); added those 3 to x86 `ASM_MNEMONIC`. #5 de-dup'd
> the kernel base into ONE `_kernel_load_base(S)` accessor (x86 + aarch64 `fixup.cyr`, replacing the
> 5-site literal the kernel-load-base-settable issue warned was an all-or-nothing garble), THEN made it
> settable via `CYRIUS_KERNEL_BASE=0x<hex>` env (`_parse_hex_env` / cell 0x18FCE8, read in main.cyr +
> main_aarch64.cyr) whose manifest surface is `[sections] base` (cbt appends the env, bare-metal only)
> + a base-aware D6 build-report. **Resolves the kernel-load-base-settable issue** (folded into #5;
> archived). **VERIFIED:** self-host fixpoint + seed→cybs→cycc byte-identical; **ELF32 + ELF64 kernels
> byte-identical pre/post** (`cmp` — the de-dup is inert); check.sh 101→102 (`_sections_base_override_gate`);
> qemu-boot-gate green + an override kernel at 0x200000 **BOOTS emitting AGNOS** (fixups followed the
> base); pi+ecb+cass SELFHOST_OK; bench 504 ms (flat); cycc 1,077,136 B; cycc-native-aarch64 regenerated
> 914,960 B + pi own-fixpoint OK. version-bump 6.3.3 run (cycc/cycc_aarch64/cycc_win rebuilt, install
> snapshot refreshed, seed-derive OK). To commit: `src/backend/{x86,aarch64}/fixup.cyr` +
> `src/backend/x86/emit.cyr` + `src/backend/common/runtime.cyr` + `src/main.cyr` + `src/main_aarch64.cyr`
> + `cbt/{core,build,commands}.cyr` + `programs/{asmtest,checks/{selfhost,platform_efi,main}}.cyr` +
> `tests/tcyr/inline_asm_fences.tcyr` + docs (CHANGELOG/roadmap/state, issue git-mv'd to archived/) +
> vidya `field_notes/kernel.cyml` + version-bump artifacts (VERSION, `src/version_str.cyr`, `build/cycc`,
> `build/cycc-native-aarch64`, CLAUDE.md, install.sh). User pushes/tags after CI. **NEXT: v6.3.4 =
> bare-metal #7** (kernel-freestanding `lib/tls_native` link + in-kernel handshake smoke). Then Phase-0
> substrate (.5) → the language trio (.6–.8).
>
