...
| sigil fold | **3.9.3** (@6.2.42 — `certpin_compute_spki_pin` obsolete 2-arg `run_capture(cmd,argv)` → current 5-arg API + output buffer; the v6.2.41 arity check surfaced it (`expects 5, got 2`), fixed upstream + released, folded byte-identical; @6.2.31 — luks raw `getrandom` → `_sigil_random_fill` portable boundary) |
...
| bench (every-release gate) | self_compile **531 ms** @ 6.2.42 per bench-history.sh (vs .41's 519 — pure jitter: cycc is **byte-identical** to .41 (lib-only sigil fold), so self_compile measures the same binary on the same src; the single-run history band is 500–549 ms); x86 cycc **1,073,560 B** (unchanged) |
