...
| check.sh gates | **93/93 + the D7 boot gate** (+1 @.45 — `_kernel_pie_struct_gate`: `kernel; --pie` → ET_DYN + PT_LOAD `p_vaddr=0`, structural-only (live boot consumer-gated); +2 @.29 — `_cli_cross_compile_gate` (CLI cbt/cyrius.cyr → PE/Mach-O/aarch64) + `_fuzz_harness_gate`; both also per-PR ci.yml steps. + the D7 boot gate post-step @.28) |
...
| bench (every-release gate) | self_compile **509 ms** @ 6.2.45 (bench-history.sh; within the 500–549 ms jitter band, flat vs .44's 513 ms — the `_entry_base` branches are cold); x86 cycc **1,073,672 B** (+112 B @ .45) |
