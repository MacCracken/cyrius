...
| | |
|---|---|
| **Version** | **6.4.23** — repair batch: **signed sub-i64 GLOBAL scalar sign-extension** (`var G: i32 = -1` now reads −1, not 4294967295; new 8th per-var `_vsgn_base` table, max-sized/no-grow to keep the cybs seed-chain intact — wiring it into the grow-chain desynced cybs) + the **agnos `sys_readlink`#70 peer**. Per-release detail → [CHANGELOG](../../CHANGELOG.md). |
| **cycc** | x86 1,073,648 B (self-host fixpoint; +4 KB from the signed-global sign-ext code; differential codegen-diff=1/status-diff=0 = contained fix) · check.sh **133** · self_compile ~0.6 s |
...
| **Next up** | **.24 = P1 struct-field-offset collision deep-dive** (cycc instrumentation — NOT the simple field collision that was filed; a size-dependent buffer/boundary bug, disproven minimally) → **.25 = the aarch64 issues** (f64 exp2/atan hard-error + trig Payne-Hanek) → then **SIMD Pin 1 aarch64 NEON — Phase 5** → async (arc 5b). cx tail: f32 + transcendentals still fail loud. |
