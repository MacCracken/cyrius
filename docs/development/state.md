...
**`build/cc3` dropped early** (user 2026-06-08, "cc3 is dead" — confirmed by direct check: never in the
bootstrap chain `bootstrap/asm → cybs → cycc`, nothing in build/CI/install loaded it). Removed from git
+ `.gitignore`; `benches/bench_capacity_overhead.sh` repointed to `cycc`; stale pre-rename `cc5_*`
scratch swept from `build/`. **`build/` now tracks only `cycc` + `cycc-native-aarch64`** (the one
cross-bin that can't be regenerated by cross-compiling). `cycc_aarch64` / `cycc_win` stay
built-on-demand (user: "keep building on demand"). The prior-major-seed policy retired a major early.
