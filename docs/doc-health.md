...
|---|---|---|---|
| `README.md` | 2026-06-08 | ✅ Fresh | **Refreshed 2026-06-08 (v6.1.0)**: current-state metrics → cycc 931,960 B at v6.1.4 (cross-compilers `cycc_aarch64` 592,512 B / `cycc_win` 804,864 B). Prior (2026-06-07, v6.0.87): 85 check.sh gates, 167 .tcyr, 90 modules. Historical "as of v5.11.x" feature-landing markers retained. |
| `CHANGELOG.md` | 2026-06-08 | ✅ Fresh | **Source of truth per CLAUDE.md.** Through **v6.1.4** (v6.1.x Phase A+B; see CHANGELOG). Refreshed every release. |
| `CLAUDE.md` | 2026-06-08 | ✅ Fresh | Process + procedures + project-identity. **2026-06-08**: Release rule #6 (benchmark every release) added; `build/` policy rewritten (cc3 dropped early). Version field at `6.1.4` (bumped by `version-bump.sh`). |
| `VERSION` | 2026-06-08 | ✅ Fresh | Single source of truth for version (`6.1.4` at last edit). Bumped via `scripts/version-bump.sh`. |
| `BENCHMARKS.md` (root) | 2026-06-08 | ✅ Fresh | Auto-generated by `scripts/bench-history.sh` (3-tier suite); history in `bench-history.csv`. **Refreshed every-release (v6.1.4)** — self_compile ~**458 ms**, cycc 931,960 B. **Benchmarking is now an every-release gate** (CLAUDE.md Release rule #6), not closeout-only. |
...
| `docs/guides/faq.md` | 2026-06-08 | ✅ Fresh | **Refreshed 2026-06-08 (v6.1.0)**: size/version markers → ~931 KB / v6.1.4; self_compile ~458 ms; i64/no-floats overclaim fixed; native-TLS + 3-platform-self-host capability added. No longer stale. |
...
| `docs/size-comparisons.md` | 2026-06-08 | ✅ Fresh | **cycc-size refreshed 2026-06-08 (v6.1.4)**: → **931,960 B** (cycc) / 592,512 B (cycc_aarch64) / 804,864 B (cycc_win). cycc vs gcc/clang/rustc binary size table (comparison-tool figures still from the 2026-05-03 sweep). |
...
| `issues/2026-05-27-yeo-cy-test-no-tsx-js-emit.md` | 2026-05-27 | 🟡 Open — TS/TSX → JS **emit** stage missing (front-end parses, no codegen). v6.1.x Phase D (roadmap.md). |
