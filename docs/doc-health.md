...
> **Last refresh**: 2026-05-27 (v6.0.3 — doc sweep across the v6.0.0→.3 band. cycc = **874,280 B** (within ±50 KB currency gate). check.sh **79/79** (76 at v6.0.0 → +2 at v6.0.2 deps correct-lock band → +1 at v6.0.3 str_from overload gate). **v6.0.x shipped so far**: v6.0.0 `cybs`+`cycc` two-binary rename; v6.0.1 stdlib-resolution hotfixes (lex skip-prefix off-by-one + cmd_deps mkdir-before-find); v6.0.2 `cyrius deps` correct-lock fix (empty ecosystem-wide since v5.11.8) + stdlib pins verified at 6.0.1; v6.0.3 `str_from` overload-misroute codegen P1 (nous 0001 — root cause corrected from the filing). **Refreshed this sweep**: README + size-comparisons + platform-status + faq (version/size markers → v6.0.3); state.md + roadmap.md (rotate every release; roadmap rewritten this session for the v6.0.2–.7 sequence + native-TLS-arc pin). Prior refresh: 2026-05-19 v5.11.69 v5.x cycle-close sweep; 2026-05-18 v5.11.63 commandress close; 2026-05-17 v5.11.59 iron-boot + DCE; 2026-05-13 cap-drift/doc-size gates.) | **Refresh cadence**: when docs are touched, update the affected row. **Programmatic gates active**: `_doc_size_currency_gate()` flags cycc-size claims outside ±50 KB of actual; `_cap_drift_gate()` cross-checks heap-map comments against inline literal caps; `_cve05_guard_gate()` locks tok_names mangle-path write boundaries (all in `programs/check.cyr`).
...
## At a glance — inventory (last reviewed 2026-05-27, v6.0.3 sweep)
...
| 🟠 **Read-through outstanding** | 1 | `docs/development/migration-strategy.md` — frozen at v5.7.39 Wave snapshot. **Its "defer to v6.0.0 doc-pass" trigger has now arrived** (v6.0.0–.3 shipped); rewrite-or-archive flagged to the project leader 2026-05-27 as its own follow-up (not a mechanical-sweep item). |
...
|---|---|---|---|
| `README.md` | 2026-05-27 | ✅ Fresh | **Refreshed 2026-05-27 (v6.0.3 sweep)**: current-state metrics → cycc 874,280 B at v6.0.3, 79 check.sh gates, 153 .tcyr; bootstrap-chain size figure updated. Historical "as of v5.11.x" feature-landing markers retained. |
...
| `docs/size-comparisons.md` | 2026-05-27 | ✅ Fresh | **cycc-size refreshed 2026-05-27 (v6.0.3)**: → **874,280 B** (cycc) / 564,512 B (cycc_aarch64). cycc vs gcc/clang/rustc binary size table. |
| `docs/platform-status.md` | 2026-05-27 | ✅ Fresh | **cycc-size marker refreshed 2026-05-27 (v6.0.3)**: ~874 KB. Per-target status table. |
