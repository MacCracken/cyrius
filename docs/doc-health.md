...
|---|---|---|---|
| `README.md` | 2026-06-05 | ✅ Fresh | **Refreshed 2026-06-05 (v6.0.70)**: current-state metrics → cycc 924,296 B at v6.0.70, 85 check.sh gates, 160 .tcyr, 89 modules; cross-compiler (`cycc_aarch64` 589,960 B / `cycc_win` 794,624 B) + bootstrap-chain figures updated. Historical "as of v5.11.x" feature-landing markers retained. |
| `CHANGELOG.md` | 2026-05-27 | ✅ Fresh | **Source of truth per CLAUDE.md.** Through **v6.0.3** (str_from overload-misroute fix). Refreshed every release. |
| `CLAUDE.md` | 2026-05-27 | ✅ Fresh | Process + procedures + project-identity. Volatile state delegated to state.md per its own principle. Version field at `6.0.3` (bumped by `version-bump.sh`). |
| `VERSION` | 2026-05-27 | ✅ Fresh | Single source of truth for version (`6.0.3` at last edit). Bumped via `scripts/version-bump.sh`. |
...
| `docs/guides/faq.md` | 2026-05-27 | ✅ Fresh | **Version/size marker refreshed 2026-05-27 (v6.0.3)**: ~874 KB at v6.0.3. self_compile ~387 ms figure retained (v5.10.40/.41 miniarc). |
...
| `docs/size-comparisons.md` | 2026-06-05 | ✅ Fresh | **cycc-size refreshed 2026-06-05 (v6.0.70)**: → **924,296 B** (cycc) / 589,960 B (cycc_aarch64) / 794,624 B (cycc_win). cycc vs gcc/clang/rustc binary size table (comparison-tool figures still from the 2026-05-03 sweep). |
| `docs/platform-status.md` | 2026-06-05 | ✅ Fresh | **cycc-size marker refreshed 2026-06-05 (v6.0.70)**: ~924 KB. Per-target status table. |
...
| `roadmap.md` | 2026-05-27 | ✅ Fresh | **Refreshed 2026-05-27 (this session)** — v6.0.x pinned slot sequence reworked: .2 stdlib+lock, .3 nous-0001 codegen, .4 kybernet aarch64-hang + DCE audit, .5 alloc+vec, .6 return-patch→vec, .7 backend collapse. v6.2.x gained the **native TLS arc** (`tls_native.cyr`, 1.2+1.3 client+server over sigil; RISC-V flagged as the split-out candidate). 6-minor v6.x structure intact. |
...
| `migration-strategy.md` | 2026-04-30 | 🟠 Read-through | **TRIGGER ARRIVED** — was deferred to "v6.0.0 doc-pass"; v6.0.0–.3 have now shipped. Still frozen at v5.7.39 Wave snapshot. Most Wave work shipped piecemeal (niyama fold v5.9.0, sandhi 1.3.3 v5.10.34, bayan/ganita carve pending mabda-3.0-GA clean-slate). Rewrite (or archive-as-historical) is now due — **flagged to project leader 2026-05-27** as its own follow-up, not folded into this mechanical sweep. |
...
| `issues/2026-05-13-build-artifact-precommit-hook.md` | 2026-05-13 | 🟡 Open — Medium severity. v5.11.45 grep gate is the catch-after-the-fact safety net; pre-commit hook would close the surface entirely. Filed during v5.11.49 vidya cleanup sweep. Roadmap: v6.0.x holdover. |
| `issues/2026-05-20-kybernet-cycc_aarch64-6.0.1-codegen-hang.md` | 2026-05-20 | 🔴 Open — **Hard regression**: `cycc_aarch64` 6.0.1 hot-spins 99.9% CPU on kybernet's full dep-bundle codegen (aarch64 cross-build unusable; x86 clean). Suspect DCE-walk / `lib/args.cyr` aarch64 path. **Pinned v6.0.4** (folds the DCE correctness audit; v6.0.3 cross-host smoke added the aarch64 *wrapper argv-dispatch* breakage as further evidence). See roadmap v6.0.4. |
