...
|---|---|---|---|
| `README.md` | 2026-06-07 | ✅ Fresh | **Refreshed 2026-06-07 (v6.0.87)**: current-state metrics → cycc 931,072 B at v6.0.87, 85 check.sh gates, 167 .tcyr, 90 modules; cross-compiler (`cycc_aarch64` 591,152 B / `cycc_win` 803,840 B) + bootstrap-chain figures updated. Historical "as of v5.11.x" feature-landing markers retained. |
| `CHANGELOG.md` | 2026-06-07 | ✅ Fresh | **Source of truth per CLAUDE.md.** Through **v6.0.87** (AGNOS getenv/envp + cross-build gate). Refreshed every release. |
| `CLAUDE.md` | 2026-06-07 | ✅ Fresh | Process + procedures + project-identity. Volatile state delegated to state.md per its own principle. Version field at `6.0.87` (bumped by `version-bump.sh`). |
| `VERSION` | 2026-06-07 | ✅ Fresh | Single source of truth for version (`6.0.87` at last edit). Bumped via `scripts/version-bump.sh`. |
...
| `docs/guides/faq.md` | 2026-05-27 | 🟡 Stale | **Version/size marker last refreshed 2026-05-27 (v6.0.3)**: ~874 KB at v6.0.3 — now stale (cycc is 931,072 B / ~909 KB at v6.0.87, well past the ±50 KB currency gate). self_compile ~387 ms figure retained (v5.10.40/.41 miniarc). Size marker due for refresh. |
...
| `docs/size-comparisons.md` | 2026-06-07 | ✅ Fresh | **cycc-size refreshed 2026-06-07 (v6.0.87)**: → **931,072 B** (cycc) / 591,152 B (cycc_aarch64) / 803,840 B (cycc_win). cycc vs gcc/clang/rustc binary size table (comparison-tool figures still from the 2026-05-03 sweep). |
| `docs/platform-status.md` | 2026-06-07 | ✅ Fresh | **cycc-size marker refreshed 2026-06-07 (v6.0.87)**: ~931 KB. Per-target status table. AGNOS-userspace row added + macOS-x86 self-host status corrected (`ach` self-hosts) at v6.0.87. Windows COM/Win64 callptr path validated on real cass (.71 + .86 stack-callee fix). |
