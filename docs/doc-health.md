...
## At a glance — inventory (bucket counts last fully re-tallied 2026-06-04 at the v6.0.62 sweep; per-tier sections below are current to the 2026-06-09 v6.1.18 sweep — the rollup counts here lag and are approximate)
...
|---|---|---|---|
| `README.md` | 2026-06-08 | ✅ Fresh | **Refreshed 2026-06-08 (v6.1.15)**: metrics block + ASCII bootstrap diagram → **cycc 1,038,584 B at v6.1.15** (cross: `cycc_aarch64` 593,384 B / `cycc_win` 805,888 B); 87 check.sh gates, 169 .tcyr, 90 modules. Capability line is feature-level (the .13–.15 bug fixes added no new capability). Historical "as of v5.11.x" feature-landing markers retained. |
| `CHANGELOG.md` | 2026-06-08 | ✅ Fresh | **Source of truth per CLAUDE.md.** Through **v6.1.15** (v6.1.x; see CHANGELOG). Refreshed every release. |
| `CLAUDE.md` | 2026-06-08 | ✅ Fresh | Process + procedures + project-identity. Release rule #6 (benchmark every release); `build/` policy (cc3 dropped early). Version field at `6.1.15` (bumped by `version-bump.sh`). |
| `VERSION` | 2026-06-08 | ✅ Fresh | Single source of truth for version (`6.1.15` at last edit). Bumped via `scripts/version-bump.sh`. |
