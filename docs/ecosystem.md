...
| `lib/sigil.cyr` | v5.8.65 (refold v6.5.36) | sigil 3.12.14 | Security (x509 + Ed25519 sign/verify — powers native TLS + cyrsign release signing; UEFI Secure Boot signing + enrollment; **RSA verify authentication-bypass closed across 3.12.3-3.12.6**, the last of which needed cyrius 6.5.14's tail-call fix to be expressible). ⛔ **3.12.14 was written during this fold** — 3.12.13's subprocess work broke agnos AND Windows: `agnosys_run_capture_timeout` was guarded for agnos only (Windows has no `SYS_FCNTL` either), and `agnosys_run_checked_timeout` — added by 3.12.13 itself — had no guard at all. sigil is in every fold's preamble, so that one line broke **11 of 12** folds on agnos and the Windows builds of mabda and yukti. Fixed at SOURCE, not in the fold. |
