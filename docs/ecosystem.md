...
| `lib/mabda.cyr` | **v6.0.45** (refold v6.6.5) | mabda 4.1.4 | GPU/compute (AMD-native GA; array textures + cubemaps + BC arrays; samvada/chitra calls `#ifdef`-gated) |
...
| `lib/ganita.cyr` | **v6.1.26** (refold v6.6.5) | ganita 1.2.6 | Linear algebra + advanced math (matrix / linalg / transcendental + fibonacci/binomial). **Carve** out of stdlib (closes Phase E): renamed `ganita_*` + legacy aliases. Keep stdlib `math` in scope (f64-exp/ln polyfills + F64 constants). |
