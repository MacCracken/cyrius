...
|-----|-----------|------------|--------|
| `lib/sandhi.cyr` | v5.7.0 (refold v6.5.21) | sandhi 1.9.10 | HTTP/2 + JSON-RPC + service discovery + TLS policy (ALPN/SPKI on typed native verbs; @1.9.10 closed a P1 where `_sandhi_resp_frame_a` returned `SANDHI_OK` carrying `body_ptr = 0` on an OOM body copy — reachable on ordinary arena-backed exchanges because `rbuf` is allocated eagerly at the full cap. ⚠ upstream's CHANGELOG also claims the chunked branch shares the defect; verified against live code at the v6.5.21 fold, it does NOT — that guard is defensive only) |
