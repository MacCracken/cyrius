...
| `lib/patra.cyr` | v5.8.65 (refold v6.5.20) | patra 1.13.0 | Storage (thread-local slots allocator-managed @1.12.12; @1.13.0 dropped its `[deps.sakshi]` 2.4.2 pin — a folded module pinning a sakshi 8 patches behind the one this snapshot ships, silently downgrading `lib/sakshi.cyr` for every transitive consumer. Code is byte-unchanged bar whitespace) |
