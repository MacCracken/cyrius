...
### Is it fast?
The compiler self-compiles in ~730 ms (cycc reproduces a byte-identical cycc, 3-step fixpoint; ~1.19 MB output — 1,247,608 B at v6.6.1 — `BENCHMARKS.md` carries the canonical, every-release figure). ⚠ Treat a single self_compile figure as noise: this box read 648–701 ms across one release with no code change between runs, and 731 vs 734 ms across two runs an hour apart at v6.6.1. Compile time fell from 1037 ms → ~387 ms (-63%, 2.7×) across the v5.10.40+v5.10.41 compile-time-perf miniarc, then settled around ~650 ms as growth-tax over the v6.0.x–v6.4.x feature work (native TLS, PIE codegen, the TS/TSX→JS emitter, Phase-0 growable tables, the v6.3 deps-model + undef-hard-error, the v6.4.x SIMD compute arc — not a regression). Programs are 10-233x smaller than GNU equivalents. `wc` is 20x faster than GNU on large files. See [size comparisons](../size-comparisons.md) for the canonical exit42 numbers across languages and platforms.
