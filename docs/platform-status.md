...
> **Last verified against live code: 2026-08-22, at v6.5.34.** Every ✅
...
|----------|--------|--------|
| Linux x86_64 | ELF | **✅ Narrow + Broad** — primary host. cycc ~1.13 MB (1,182,944 B at v6.5.34; x86 packed-SIMD/AVX2 emitters in float.cyr; SIMD Phase 5 complete on all four backends — x86 SSE/AVX2, aarch64 NEON, Win64 PE, cx); 3-step bootstrap byte-identical; **W^X** by default since v6.3.12 (text `R E` / data `RW ` `PT_LOAD` split, `CYRIUS_WX=0` opts out). |
...
**v6.5.34** all four report `SELFHOST_OK` + `LIBTEST_OK` (crossos **54/54** each, on real
hardware). ⚠ **That is the 54-file `crossos/` subset, not the full corpus** — the
