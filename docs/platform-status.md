...
|----------|--------|--------|
| Linux x86_64 | ELF | **✅ Narrow + Broad** — primary host. cycc ~1.19 MB (1,247,608 B at v6.6.1; x86 packed-SIMD/AVX2 emitters in float.cyr; SIMD Phase 5 complete on all four backends — x86 SSE/AVX2, aarch64 NEON, Win64 PE, cx); 3-step bootstrap byte-identical; **W^X** by default since v6.3.12 (text `R E` / data `RW ` `PT_LOAD` split, `CYRIUS_WX=0` opts out). ✅ `CYRIUS_DCE=1` performs REAL elimination here — the only target that shrinks (123,048 → 16,552 B measured on the v6.6.1 repro). |
...
| cyrius-x bytecode | .cyx | **✅ Narrow + Broad** — clean CYX bytecode; portable-target arc complete (v6.4.20): a `.cyx` doing I/O runs on all 4 hosts. Full SIMD codegen (per-lane emitters for every flat-array verb, `_CX_VLOOP_BIN`, cxvm opcodes through 0x68, v6.4.32) + 64-bit immediates (opcode 253 `movhk`, v6.4.58). ⚠ **No indirect call**: `callptr` is a hard compile error on cx and `fncall*` silently returns 0, so every fn-pointer stdlib path (allocator vtables, callbacks, `vec_sort_by`) is dead there — `docs/development/issues/archived/2026-07-30-cx-backend-has-no-indirect-call.md` — archived into the backlog, still reads OPEN, every claim re-reproduced on 6.5.10. |
