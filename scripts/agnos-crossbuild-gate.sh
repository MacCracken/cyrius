...
fn main(): i64 {
    ...
    var r13 = sys_gpu_recover_op(RECOV_ARM_C);     # #94 recovery battery, arm C = the NEGATIVE control
                                                   #     (LCHOWN on x86 — arg1 read as a PATH;
                                                   #      **EXIT_GROUP on aarch64 — TERMINATES**)
    var r14 = sys_uptime_us();                     # #95 rdtsc µs clock (UMASK on x86 — nullary, so the
                                                   #     Linux call SUCCEEDS and returns a plausible number)
    return r + r2 + r3 + r4 + r5 + r6 + r7 + r8 + r9 + r10 + r11 + r12 + r13 + r14;
}
CYR
...
if command -v objdump >/dev/null 2>&1; then
    ...
    for want in 0x52:82:SYS_GPU_DISPATCH 0x53:83:SYS_GPU_DISPATCH_F64 0x54:84:SYS_GPU_PRESENT 0x55:85:SYS_GPU_FILL \
                ...
                0x5c:92:SYS_GPU_SHADER_OP 0x5d:93:SYS_GPU_MODESET_OP \
                0x5e:94:SYS_GPU_RECOVER_OP 0x5f:95:SYS_UPTIME_US; do
