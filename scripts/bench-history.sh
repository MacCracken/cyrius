...

{
    ...
    echo ""
    echo "> ⚠ **Micro rows recorded before v6.5.19 are the timer, not the code.**"
    echo "> \`bench_run\` wrapped a clock pair around every iteration, so 57 of 79 micro"
    echo "> rows sat on a ~1,320-1,720 ns floor on this box. v6.5.19 measures and"
    echo "> subtracts that floor, which produces a one-off drop of one to three orders of"
    echo "> magnitude in those rows -- expected, not a perf win, and not comparable across"
    echo "> the boundary. \`compiler/*\` and \`size/*\` are timed by \`bench_cmd\` and are"
    echo "> unaffected in every regime. See \`docs/development/benchmark-regimes.md\`."
    echo ""
    echo "## Run History"
    echo ""
