...
# Recognizes two marker styles:
#   1. `# Generated by: cyrius distlib` in the first 5 lines (cyrius
#      distlib output — patra, sigil, vani, yukti, sankoch, agnosys,
#      mabda, sandhi, etc.).
...
# Either marker means the file is byte-snapshot vendored from upstream
# and re-formatting it would diverge from the upstream byte-identity.
_aw_is_distlib_bundle() {
    head -n 5 "$1" 2>/dev/null | grep -qE "^# Generated by: cyrius distlib$|^# Bundled distribution of "
}
