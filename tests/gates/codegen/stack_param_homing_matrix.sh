...
awk -v out="$T/probe.cyr" -v rowsf="$T/rows" '
...
if [ "$ROWS" -lt "$ROWS_FLOOR" ]; then
    echo "  FAIL: stack_param_homing: generated only $ROWS rows (floor $ROWS_FLOOR) — the matrix shrank"; exit 1
fi
RAN_WANT=$(printf 'R%03d' "$ROWS")

# run_leg <label> <program-file> <runner...>: the program must print exactly R<rows> and exit 0
run_leg() {
    _l=$1; _b=$2; shift 2
...
else chmod +x "$T/p.x86"; run_leg x86 "$T/p.x86" direct; fi
...
if command -v qemu-aarch64 > /dev/null 2>&1; then
    ...
    if [ ! -s "$T/cc_a64" ]; then echo "  FAIL: [aarch64] could not build the aarch64 cross-compiler"; fail=1
         ...
         qa() { timeout 60 qemu-aarch64 "$1"; }; run_leg aarch64-qemu "$T/p.a64" qa; fi
...
else "$T/cc_cx" < "$T/probe.cyr" > "$T/p.cyx" 2> /dev/null || true
     cxr() { timeout 60 "$T/cxvm" < "$1"; }; run_leg cx "$T/p.cyx" cxr; fi
...
if command -v wine > /dev/null 2>&1; then
    ...
    if [ ! -s "$T/cc_win" ]; then echo "  FAIL: [win64] could not build the PE cross-compiler"; fail=1
         ...
         else wr_() { timeout 180 wine "$1"; }; run_leg win64-wine "$T/p.exe" wr_; fi
...
echo "PASS stack_param_homing_matrix: $ROWS generated rows (4 vector classes x 3 positions x 5..9 int args, 2 vectors + 7 ints, struct return x 5..9) bind every argument on every leg that ran"
