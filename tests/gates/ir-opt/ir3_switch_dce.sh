...
cd "$ROOT" || exit 2
CC="$ROOT/build/cycc"
D=$(mktemp -d) && [ -d "$D" ] || { echo "FAIL: ir3_switch_dce: mktemp -d failed (TMPDIR=${TMPDIR:-/tmp})"; exit 1; }
trap 'rm -rf "$D"' EXIT
fails=0
...
echo "axis 2 — tests/tcyr/codegen/switch_dispatch.tcyr under CYRIUS_IR=3 (want 0, was 18):"
cat tests/tcyr/codegen/switch_dispatch.tcyr | CYRIUS_IR=3 "$CC" > "$D/sd" 2>/dev/null
...
n_ok=0; n_bad=0
for f in tests/tcyr/codegen/switch_dispatch.tcyr tests/tcyr/stdlib/vec_sort.tcyr programs/cyrfmt.cyr; do
