#!/bin/sh
# tests/reloadelim.sh — v6.3.30 redundant-reload elimination gate (O2 category 5).
#
...
cat tests/tcyr/reload_elim.tcyr | build/cycc > "$TMP/re_on" 2>/dev/null; chmod +x "$TMP/re_on" 2>/dev/null
"$TMP/re_on" >/dev/null 2>&1 || { echo "FAIL: reload_elim.tcyr hazard suite (reload-elim on)"; exit 1; }
CYRIUS_RELOADELIM=0 sh -c "cat tests/tcyr/reload_elim.tcyr | build/cycc > \"$TMP/re_off\"" 2>/dev/null; chmod +x "$TMP/re_off" 2>/dev/null
...
for t in tests/tcyr/reload_elim.tcyr tests/tcyr/core.tcyr tests/tcyr/closures.tcyr \
         tests/tcyr/bigint.tcyr tests/tcyr/alloc_collections.tcyr tests/tcyr/chrono.tcyr; do
    [ -f "$t" ] || continue
